...
class ArticleAiQualityScorerV2Test extends TestCase
{
    ...
    {
        $scorer = new ArticleAiQualityScorerV2;
        $retired = [[
        ...
        $withRetired = $scorer->score($this->qualityResult($retired), 85, 70);
        $without = $scorer->score($this->qualityResult([]), 85, 70);

        // 白名单外的 code 不再让整次质检作废（不崩），但必须 fail-closed：
        // 记为需人工复核的门禁理由，并照常按自报严重度参与判定。
        $this->assertFalse($withRetired['issues'][0]['code_recognized']);
        $this->assertContains('unrecognized_issue_code', $withRetired['gate_reasons']);
