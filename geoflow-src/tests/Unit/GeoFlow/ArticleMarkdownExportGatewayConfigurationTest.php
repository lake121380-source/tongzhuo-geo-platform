...
final class ArticleMarkdownExportGatewayConfigurationTest extends TestCase
{
    public function test_nginx_limits_the_prepare_request_before_php_parses_it(): void
    {
        ...
        // 2026-09-12 退役改指向：旧 Blade 后台那条 `articles/batch/export-markdown/prepare`
        // 已随退役删除，规则改守新接口。**别改回旧路径**——旧路径不存在了，改回去等于空守。
        self::assertStringContainsString(
            'location ~ ^/api/v\d+/articles/markdown-export/prepare$',
            $configuration,
        );
        self::assertStringContainsString('client_max_body_size 128k;', $configuration);
        // 反向断言：旧路径不该再出现在规则里。
        self::assertStringNotContainsString(
            'articles/batch/export-markdown/prepare$',
            $configuration,
        );
    }
