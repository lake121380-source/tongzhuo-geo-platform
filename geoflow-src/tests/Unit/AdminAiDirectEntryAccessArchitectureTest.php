...
final class AdminAiDirectEntryAccessArchitectureTest extends TestCase
{
    ...
    {
        // 真正出站的入口：模型解析已从控制器收敛到 ArticleEditorAssistantService，
        // 守卫跟着实现走，而不是跟着旧文件走。
        foreach ([
            ArticleEditorAssistantService::class,
        ...
        // 入口控制器（api/v1）只做适配，不许碰全局模型池。
        // 旧 Blade 后台那个入口控制器已随退役删除（2026-09-13），这条不变量对剩下的入口仍然成立。
        foreach ([
            ArticleEditorAssistantApiController::class,
        ] as $controller) {
            $source = $this->classSource($controller);

            $this->assertDoesNotMatchRegularExpression('/\bAiModel::(?:query|where|find|findOrFail)\s*\(/', $source, $controller);
            $this->assertDoesNotMatchRegularExpression('/->(?:aiModel|qualityModel)\b/', $source, $controller);
        }
    }
    ...
    {
        $source = $this->methodSource(ArticleEditorAssistantService::class, 'stream');
        ...
        $replacement = strpos($source, 'self::EVENT_REPLACEMENT', $iteration === false ? 0 : $iteration);
        $success = strpos($source, 'streamSession->complete(', $replacement === false ? 0 : $replacement);
        $done = strpos($source, 'self::EVENT_DONE', $success === false ? 0 : $success);
    ...
    /**
     * 入口不许自己重新实现这条流——否则上面那条顺序守卫就形同虚设。
     * 旧 Blade 后台那个入口已随退役删除（2026-09-13），不变量对剩下的 api/v1 入口仍然成立。
     */
    #[Test]
    public function article_editor_entries_delegate_instead_of_reimplementing_the_stream(): void
    {
        foreach ([
            ArticleEditorAssistantApiController::class,
        ] as $controller) {
            ...
            $this->assertStringContainsString('ArticleEditorAssistantService', $source, $controller);
            $this->assertStringNotContainsString('deferredStreamWithReservation(', $source, $controller);
            $this->assertStringNotContainsString('invocationGateway->acquire(', $source, $controller);
            $this->assertStringNotContainsString('streamSession->complete(', $source, $controller);
        }
    }
