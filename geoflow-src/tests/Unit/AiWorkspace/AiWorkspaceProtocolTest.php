...
{
    public function test_catalog_entries_have_stable_complete_contracts_and_valid_paths(): void
    {
        ...
        foreach ($entries as $entry) {
            ...
            // 2026-09-12 退役：入口从「Blade 路由名（须 router->has））」改为「React 后台的页签深链」，
            // 判据随之变成**路径形状**——旧后台的路由已经不存在，再查 router->has 只会全红。
            self::assertNotNull(
                AdminTabs::tabFromPath((string) $entry['path']),
                'Invalid catalog entry path: '.$entry['path'],
            );
        }
    }
    ...
    public function test_search_ranks_relevant_entries_and_returns_at_most_five(): void
    {
        ...
        self::assertSame(
            // 2026-09-12 退役迁移时新增了 featured 的「总览」条目，它占据了兜底 Top-5 的一席。
            ['dashboard', 'data-center', 'ai-visibility', 'tasks', 'articles'],
