...
class AiVisibilityServiceTest extends TestCase
{
    ...
    public function test_collection_asks_the_operator_query_verbatim_instead_of_a_meta_analysis_prompt(): void
    {
        MarkdownContentWriterAgent::fake(['推荐桐灼GEO，另有其他服务商可供比较。'])->preventStrayPrompts();

        $model = $this->createAiModel();
        $this->bindModel(AiVisibilityConfigurationResolver::DEEPSEEK_MODEL_SETTING_KEY, $model);

        $runs = app(AiVisibilityCollector::class)->collect(
            SystemAiIdentity::visibilityCollection(),
            '国内做 GEO 的公司有哪些',
        );

        $run = $runs['analysis_run'] ?? null;
        $this->assertInstanceOf(AiVisibilityRun::class, $run);
        $this->assertSame('国内做 GEO 的公司有哪些', (string) $run->prompt);
        $this->assertSame('推荐桐灼GEO，另有其他服务商可供比较。', (string) $run->answer_text);
        $this->assertSame(AiVisibilityRun::STATUS_COMPLETED, $run->status);
    }

    public function test_search_then_analysis_asks_the_query_instead_of_requesting_a_visibility_report(): void
    {
        Http::preventStrayRequests();
        Http::fake([
            'https://open.feedcoopapi.com/search_api/web_search' => Http::response([
        ...
        MarkdownContentWriterAgent::fake(['推荐桐灼GEO，另有其他服务商。'])->preventStrayPrompts();

        $provider = $this->createSearchProvider();
        $model = $this->createAiModel();
        $this->bindModel(AiVisibilityConfigurationResolver::DEEPSEEK_MODEL_SETTING_KEY, $model);

        $runs = app(AiVisibilityService::class)->runDoubaoSearchThenDeepSeekAnalysis(
            SystemAiIdentity::visibilityCollection(),
            $provider,
            $model,
            '国内做 GEO 的公司有哪些',
        ...
        // 分析步骤必须把运营方的问题当成问题问出去（品牌提及率就是在它的回答上算的），
        // 不能退回到“请分析该关键词的可见性并给投放建议”那种让模型写作文的形态。
        $prompt = (string) $runs['analysis_run']->prompt;
        $this->assertStringContainsString('国内做 GEO 的公司有哪些', $prompt);
        $this->assertStringNotContainsString('请分析目标关键词', $prompt);
        $this->assertStringNotContainsString('建议优先投放', $prompt);
    }
