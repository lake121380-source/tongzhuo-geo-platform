...
final class AdminTitleGenerationAiExecutionIdentityTest extends TestCase
{
    ...
    public function test_submission_freezes_authoritative_admin_and_requested_model_identity(): void
    {
        ...
        $this->withToken($this->token($admin, 'title-submit-write'))
            ->withHeader('X-Idempotency-Key', 'title-submit-identity-1')
            ->postJson(
                '/api/v1/materials/title-libraries/'.$titleLibrary->id.'/ai-generation-runs',
    ...
    public function test_another_admin_cannot_guess_status_cancel_or_retry_urls(): void
    {
        ...
        $peerToken = $this->token($peer, 'title-run-peer-write');
        $runUrl = '/api/v1/materials/title-libraries/'.$titleLibrary->id.'/ai-generation-runs/'.$run->id;

        $this->withToken($peerToken)->getJson($runUrl)->assertNotFound();
        $this->withToken($peerToken)
            ->withHeader('X-Idempotency-Key', 'title-peer-cancel-1')
            ->postJson($runUrl.'/cancel')
            ->assertNotFound();
        ...
        $this->withToken($peerToken)
            ->withHeader('X-Idempotency-Key', 'title-peer-retry-1')
            ->postJson($runUrl.'/retry')
            ->assertNotFound();

        $this->withToken($this->token($admin, 'title-run-owner-read'))->getJson($runUrl)->assertOk();
    }
    ...
    public function test_unowned_legacy_run_id_is_not_visible_to_any_admin(): void
    {
        ...
        $this->withToken($this->token($admin, 'title-unowned-read'))
            ->getJson('/api/v1/materials/title-libraries/'.$titleLibrary->id.'/ai-generation-runs/'.$run->id)
            ->assertNotFound();
    }
    ...
    public function test_retry_keeps_original_identity_and_rejects_revoked_access_version(): void
    {
        ...
        $this->withToken($this->token($admin, 'title-retry-owner-write'))
            ->withHeader('X-Idempotency-Key', 'title-retry-revoked-1')
            ->postJson('/api/v1/materials/title-libraries/'.$titleLibrary->id.'/ai-generation-runs/'.$run->id.'/retry')
            ->assertForbidden()
