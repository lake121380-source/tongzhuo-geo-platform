...
final class SystemKnowledgeBaseTest extends TestCase
{
    ...
    public function test_sync_command_installs_the_ai_workspace_system_knowledge_once(): void
    {
        ...
        self::assertSame('桐灼GEO 后台功能与操作指南（AI 工作台专用）', $knowledgeBase->name);
    ...
    public function test_system_knowledge_cannot_be_deleted_through_web_or_material_service(): void
    {
        ...
        $this->withToken($this->token($admin, 'system-knowledge-owner-write'))
            ->deleteJson('/api/v1/materials/knowledge-bases/'.$knowledgeBase->getKey())
            ->assertStatus(409)
            ->assertJsonPath('error.code', 'system_knowledge_protected');
    ...
    public function test_only_protected_admins_can_edit_and_restore_system_knowledge(): void
    {
        ...
        try {
            $manager->update($knowledgeBase, $regularAdmin, $payload);
            self::fail('Only protected workflow administrators may edit system knowledge.');
        ...
        $manager->update($knowledgeBase, $superAdmin, $payload);
        ...
        // 旧后台知识库详情页（Blade）的「官方/当前/差异」三块在 api/v1 上没有等价投影：
        // GET /api/v1/knowledge-bases/{id}/revisions/{revision} 只返回单条修订正文。
        // 原 assertSee(system_diff_*) 随 Blade 页面退役。见任务报告。
        $originalRevision = $knowledgeBase->revisions()->reorder()->orderBy('revision_number')->firstOrFail();

        $this->withToken($this->token($superAdmin, 'system-knowledge-restore-write'))
            ->postJson('/api/v1/knowledge-bases/'.$knowledgeBase->getKey().'/revisions/'.$originalRevision->getKey().'/restore')
            ->assertOk()
            ->assertJsonPath('data.revision.restored_from_revision_id', (int) $originalRevision->getKey());
    ...
    public function test_system_metadata_and_manual_reindex_are_protected(): void
    {
        ...
        $this->withToken($this->token($regularAdmin, 'knowledge-metadata-reader-write'))
            ->postJson('/api/v1/knowledge-bases/'.$knowledgeBase->getKey().'/refresh')
            ->assertForbidden();

        app(SystemKnowledgeBaseManager::class)->update($knowledgeBase, $superAdmin, [
            'name' => '伪造的系统知识名称',
    ...
    public function test_system_knowledge_rejects_unsafe_route_directives(): void
    {
        ...
        self::assertStringContainsString('[[tab:', $officialContent);
        // 站内入口指令已由 `[[route:admin.x|标签]]` 改为 `[[tab:<id>|标签]]`；一个不存在于
        // 帮助目录的 tab id 就是现在意义上的「不安全的 route 指令」。
        $unsafeContent = $officialContent."\n\n相关入口：[[tab:unsafe-admin-entry|打开动态任务]]\n";
        ...
        try {
            ...
            self::fail('Unsafe tab directives must be rejected.');
        } catch (\RuntimeException $exception) {
            self::assertStringContainsString('tab', $exception->getMessage());
        }

        self::assertSame(1, $knowledgeBase->revisions()->count());
        self::assertSame($officialContent, (string) $knowledgeBase->fresh()->content);
    }
    ...
    public function test_system_knowledge_rejects_embedded_images_external_links_and_secret_like_content(): void
    {
        ...
        foreach ([
            ...
            try {
                ...
                self::fail('Unsafe system knowledge content must be rejected.');
            } catch (\RuntimeException $exception) {
                self::assertNotSame('', $exception->getMessage());
            }
        }
    ...
    public function test_system_knowledge_edit_requires_every_official_route_entry(): void
    {
        ...
        try {
            ...
            self::fail('System knowledge must keep every official tab entry.');
        } catch (\RuntimeException $exception) {
            self::assertNotSame('', $exception->getMessage());
        }
