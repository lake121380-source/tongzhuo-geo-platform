...
class AdminKnowledgeFactAiExecutionIdentityTest extends TestCase
{
    ...
    public function test_regular_admin_can_start_generation_with_a_personal_requested_model_and_freezes_identity(): void
    {
        ...
        $response = $this->withToken($this->token($admin, 'knowledge-personal-write'))
            ->withHeader('X-Idempotency-Key', 'knowledge-personal-generation-1')
            ->postJson(
                '/api/v1/materials/knowledge-bases/'.$base->id.'/fact-generation',
                [
                    'mode' => 'initial',
                    'target_count' => 1,
    ...
    public function test_regular_admin_can_start_generation_with_an_explicit_shared_model(): void
    {
        ...
        $this->withToken($this->token($admin, 'knowledge-shared-write'))
            ->withHeader('X-Idempotency-Key', 'knowledge-shared-generation-1')
            ->postJson(
                '/api/v1/materials/knowledge-bases/'.$base->id.'/fact-generation',
                [
                    'mode' => 'initial',
    ...
    public function test_generation_start_hides_peer_system_and_regular_admin_private_model_ids(): void
    {
        ...
        foreach ([$peerModel, $systemModel, $superPrivate] as $index => $model) {
            ...
            $response = $this->withToken($this->token($admin, 'knowledge-isolated-write-'.$index))
                ->withHeader('X-Idempotency-Key', 'knowledge-isolated-generation-'.$index)
                ->postJson(
                    '/api/v1/materials/knowledge-bases/'.$base->id.'/fact-generation',
                    [
                        'mode' => 'initial',
        ...
        $this->withToken($this->token($super, 'knowledge-super-isolated-write'))
            ->withHeader('X-Idempotency-Key', 'knowledge-super-isolated-generation-1')
            ->postJson(
                '/api/v1/materials/knowledge-bases/'.$base->id.'/fact-generation',
                [
                    'mode' => 'initial',
    ...
    public function test_queue_snapshot_public_json_and_failure_error_exclude_credentials_endpoint_prompt_and_evidence(): void
    {
        ...
        $publicJson = $this->withToken($this->token($admin, 'knowledge-secret-read'))->getJson(
            '/api/v1/materials/knowledge-bases/'.$base->id.'/fact-generation/'.$run->id,
        )->assertOk()->getContent();
