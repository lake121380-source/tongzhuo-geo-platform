...
class AiQualityRetrievalReadinessServiceTest extends TestCase
{
    ...
    public function test_high_risk_knowledge_reviewed_through_the_admin_form_is_available_for_broad_quality(): void
    {
        ...
        $token = $admin->createToken('high-risk-knowledge-base', ['materials:write'])->plainTextToken;

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'high-risk-knowledge-base-'.$admin->id)
            ->postJson('/api/v1/knowledge-bases/upload', [
                'name' => '已审核高风险知识库',
