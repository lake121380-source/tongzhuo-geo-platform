...
class AdminHeaderNotificationTest extends TestCase
{
    ...
    public function test_admin_header_shows_update_indicator_when_github_version_is_newer(): void
    {
        ...
        $token = $admin->createToken('update-indicator', ['system:read'])->plainTextToken;

        // 旧 Blade 头部通知已退役，改指 React 后台头部实际消费的投影：
        // `GET /api/v1/system-updates` 返回同一份 buildNotificationPayload()，
        // 断言的仍然是「远端版本更新时管理员被告知新版本与摘要」这一行为。
        $this->withToken($token)
            ->getJson('/api/v1/system-updates')
            ->assertOk()
            ->assertJsonPath('data.release.state.status', 'available')
            ->assertJsonPath('data.release.state.is_update_available', true)
            ->assertJsonPath('data.release.state.latest_version', '2.1')
            ->assertJsonPath('data.release.notice.available', true)
            ->assertJsonPath('data.release.notice.summary', '测试更新摘要');
    }
