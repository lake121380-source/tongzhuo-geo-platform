...
class DistributionChannelDeletionServiceTest extends TestCase
{
    ...
    public function test_manual_retry_cannot_requeue_work_for_a_deleting_channel(): void
    {
        ...
        $this->withToken($this->distributionToken())
            ->withHeader('X-Idempotency-Key', 'retry-deleting-channel')
            ->postJson('/api/v1/distribution/jobs/'.(int) $distribution->id.'/retry')
            ->assertStatus(409)
            ->assertJsonPath('error.code', 'distribution_channel_unavailable');
    ...
    public function test_manual_retry_does_not_overwrite_a_sending_distribution(): void
    {
        ...
        $this->withToken($this->distributionToken())
            ->withHeader('X-Idempotency-Key', 'retry-must-not-overwrite-sending')
            ->postJson('/api/v1/distribution/jobs/'.(int) $distribution->id.'/retry')
            ->assertStatus(409)
            ->assertJsonPath('error.code', 'distribution_retry_in_progress');
    ...
    /**
     * 旧后台 `admin.distribution.retry` 的等价 API 入口是
     * `POST /distribution/jobs/{id}/retry`（scope：articles:publish + distribution:write）。
     */
    private function distributionToken(): string
    {
        return $this->admin()
            ->createToken('distribution-retry', ['articles:publish', 'distribution:write'])
            ->plainTextToken;
    }
