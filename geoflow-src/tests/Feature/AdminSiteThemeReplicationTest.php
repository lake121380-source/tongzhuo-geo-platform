...
class AdminSiteThemeReplicationTest extends TestCase
{
    ...
    public function test_admin_can_create_theme_replication_task(): void
    {
        ...
        $response = $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-store-created')
            ->postJson('/api/v1/site-settings/theme-replications', $this->storePayload($model->id, [
                'name' => 'Brand Clone',
        ...
        $response->assertCreated()
            ->assertJsonPath('data.replication.id', $replication->id)
    ...
    public function test_theme_replication_store_returns_validation_error_when_tables_are_missing(): void
    {
        ...
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-missing-tables')
            ->postJson('/api/v1/site-settings/theme-replications', $this->storePayload($model->id, [
                'name' => 'Missing Tables Clone',
                'theme_id' => 'missing-tables-clone',
            ]))
            ->assertStatus(409)
            ->assertJsonPath('error.code', 'theme_replication_migration_required')
            ->assertJsonPath('error.message', __('admin.theme_replication.message.migration_required'));
    ...
    {
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-private-url')
            ->postJson('/api/v1/site-settings/theme-replications', $this->storePayload($this->activeChatModel()->id, [
                'name' => 'Private Clone',
            ...
            ->assertStatus(422)
            ->assertJsonPath('error.code', 'theme_replication_create_failed')
            ->assertJsonPath('error.message', __('admin.theme_replication.validation.url_private'));
    ...
    {
        // 私有地址在字段层通不过：服务层把命中的字段名带进错误详情。
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-private-category-url')
            ->postJson('/api/v1/site-settings/theme-replications', $this->storePayload($this->activeChatModel()->id, [
    ...
    {
        // 2026-09-12 退役改指：旧 Blade 的 store 会校验基底主题是否属于主题目录，
        // 这条 API 原先把 `base_theme_id` 只当字符串收（静默接受未知值）——已补齐同一条边界，
        // 断言随之从「会话错误」改为 API 的字段级错误。
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-replication-unknown-base')
            ->postJson('/api/v1/site-settings/theme-replications', [
            ...
            ->assertJsonPath('error.details.field_errors.base_theme_id', __('admin.theme_replication.validation.base_theme_invalid'));
    ...
    public function test_theme_replication_requires_active_chat_model(): void
    {
        ...
        // api/v1 的 ai_model_id 规则只保留 exists+chat 约束，未接上旧后台的
        // admin.theme_replication.validation.ai_model_required 文案——断言校验确实
        // 落在 ai_model_id 上、错误码是 validation_failed，而不是放过这个 embedding 模型。
        $response = $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-invalid-model')
            ->postJson('/api/v1/site-settings/theme-replications', $this->storePayload($embeddingModel->id, [
        ...
        $this->assertStringStartsWith(
            'The selected ai model id is invalid.',
            (string) $response->json('error.details.field_errors.ai_model_id'),
        );
    }
    ...
    public function test_theme_replication_status_endpoint_returns_progress_payload(): void
    {
        ...
        $this->withToken($this->token())
            ->getJson('/api/v1/site-settings/theme-replications/'.$replication->id)
            ->assertOk()
            ->assertJsonPath('data.replication.progress.status', SiteThemeReplication::STATUS_GENERATING)
            ->assertJsonPath('data.replication.progress.current_step', 'generating')
            ->assertJsonPath('data.replication.progress.terminal', false)
            ->assertJsonPath('data.replication.progress.stages.5.key', 'generating')
            ->assertJsonPath('data.replication.progress.logs.0.step', 'generating');
    }
    ...
    public function test_failed_iteration_progress_keeps_reference_stage_when_refetching(): void
    {
        ...
        $this->withToken($this->token())
            ->getJson('/api/v1/site-settings/theme-replications/'.$replication->id)
            ->assertOk()
            ->assertJsonPath('data.replication.progress.current_step', 'fetching')
            ->assertJsonPath('data.replication.progress.stages.3.key', 'fetching')
            ->assertJsonPath('data.replication.progress.stages.3.state', 'failed');
    }
    ...
    public function test_failed_theme_replication_can_be_retried(): void
    {
        ...
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-retry-failed')
            ->postJson('/api/v1/site-settings/theme-replications/'.$replication->id.'/retry')
            ->assertOk()
            ->assertJsonPath('data.replication.status', SiteThemeReplication::STATUS_QUEUED)
    ...
    public function test_ready_theme_replication_preview_uses_only_trusted_static_content(): void
    {
        ...
        foreach (['home', 'category', 'article'] as $page) {
            // 2026-09-12 退役改指：预览接口从 Blade 路由迁到
            // `GET /api/v1/site-settings/theme-replications/{id}/preview/{page}`（React 后台调的就是它）。
            // 断言的**安全性质一条没减**：CSP sandbox、nosniff、no-store，以及不得出现
            // `@php` / `{!!}` / `<script` / `onerror` / `@import` / `raw-danger`。
            $this->withToken($this->token())
                ->get('/api/v1/site-settings/theme-replications/'.$replication->id.'/preview/'.$page)
                ->assertOk()
    ...
    public function test_ready_theme_replication_can_be_copied_as_new_template(): void
    {
        ...
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-copy-ready')
            ->postJson('/api/v1/site-settings/theme-replications/'.$replication->id.'/copy', [
                'name' => 'Copied Clone',
    ...
    public function test_ready_theme_replication_can_queue_feedback_iteration(): void
    {
        ...
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-iterate-ready')
            ->postJson('/api/v1/site-settings/theme-replications/'.$replication->id.'/iterate', [
                'feedback' => '让首页卡片更紧凑，标题更醒目。',
            ])
            ->assertOk()
            ->assertJsonPath('data.replication.status', SiteThemeReplication::STATUS_ITERATING);
    ...
    public function test_package_capability_is_consistent_across_model_ui_controller_and_service(
        ...
        $showResponse = $this->withToken($this->token())
            ->getJson('/api/v1/site-settings/theme-replications/'.$replication->id);
        $downloadUrl = '/api/v1/site-settings/theme-replications/'.$replication->id.'/package';
        $showResponse->assertOk()->assertJsonPath('data.replication.can_package', $expected);
    ...
    public function test_package_capability_requires_current_safe_draft_metadata(string $mode): void
    {
        ...
        // API 的 can_package 投影必须与模型的 canPackage() 一致——前端据此决定是否
        // 暴露下载入口；canPublish() 只看 status/compliance，与草稿清单无关，故不在此断言。
        $this->withToken($this->token())
            ->getJson('/api/v1/site-settings/theme-replications/'.$replication->id)
            ->assertOk()
            ->assertJsonPath('data.replication.can_package', false);
    }
    ...
    public function test_theme_replication_package_rejects_abnormal_storage_filename_and_cleans_partial_zip(): void
    {
        ...
        // Keep the malformed filename portable: Windows treats `C:` as a
        // drive-qualified directory and Flysystem fails before the service
        // can exercise its path guard.
        $unsafePath = trim((string) $version->draft_views_path, '/').'/ｅscape.blade.php';
    ...
    public function test_published_theme_replication_can_be_archived_and_draft_files_deleted(): void
    {
        ...
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-archive')
            ->postJson('/api/v1/site-settings/theme-replications/'.$replication->id.'/archive')
            ->assertOk()
            ->assertJsonPath('data.replication.status', SiteThemeReplication::STATUS_ARCHIVED);
        ...
        $this->withToken($this->token())
            ->withHeader('X-Idempotency-Key', 'theme-delete-drafts')
            ->postJson('/api/v1/site-settings/theme-replications/'.$replication->id.'/delete-drafts')
            ->assertOk();
        ...
        $this->withToken($this->token())
            ...
            ->assertJsonPath('data.replication.can_package', false);
        $this->withToken($this->token())
            ->get('/api/v1/site-settings/theme-replications/'.$replication->id.'/package')
            ->assertStatus(409);
    }
    ...
    {
        return $this->admin()->createToken('theme-rep', ['seo:read', 'seo:write'])->plainTextToken;
    }

    /** @return array<string, mixed> */
    private function storePayload(int $modelId, array $overrides = []): array
    {
        return array_merge([
