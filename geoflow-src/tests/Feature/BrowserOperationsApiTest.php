...
class BrowserOperationsApiTest extends TestCase
{
    ...
    public function test_admin_can_approve_device_and_extension_receives_one_time_browser_token(): void
    {
        ...
        $this->withToken($this->token($admin, ['articles:read']))
            ->getJson('/api/v1/manual-publications/browser-connect?user_code='.$userCode)
            ->assertOk()
            ->assertJsonPath('data.user_code', $userCode)
            ->assertJsonPath('data.authorization.status', 'pending');

        $this->withToken($this->token($admin, ['articles:write']))
            ->withHeader('X-Idempotency-Key', 'browser-connect-approve')
    ...
    public function test_device_authorization_can_be_denied_or_expire(): void
    {
        ...
        $denied = $this->withHeaders($headers)
            ...
            ->assertOk();
        $this->withToken($this->token($admin, ['articles:write']))
            ->withHeader('X-Idempotency-Key', 'browser-connect-deny')
            ->postJson('/api/v1/manual-publications/browser-connect/decision', [
    ...
    public function test_admin_only_sees_and_revokes_owned_browser_connections(): void
    {
        ...
        $owned = $this->withToken($this->token($admin, ['account:read']))
            ->getJson('/api/v1/admin/browser-clients')
            ->assertOk();
        self::assertSame(['Chrome owned'], collect($owned->json('data.items'))->pluck('name')->all());

        $all = $this->withToken($this->token($superAdmin, ['account:read']))
            ->getJson('/api/v1/admin/browser-clients')
            ->assertOk();
        $allNames = collect($all->json('data.items'))->pluck('name')->sort()->values()->all();
        self::assertSame(['Chrome other', 'Chrome owned'], $allNames);

        $this->withToken($this->token($admin, ['account:write']))
            ->withHeader('X-Idempotency-Key', 'revoke-browser-client-other')
            ->deleteJson('/api/v1/admin/browser-clients/'.$otherBrowser->id)
            ->assertNotFound();

        $this->withToken($this->token($admin, ['account:write']))
            ->withHeader('X-Idempotency-Key', 'revoke-browser-client-own')
            ->deleteJson('/api/v1/admin/browser-clients/'.$ownBrowser->id)
            ->assertOk();
    ...
    /**
     * 2026-09-12 退役改指：运营方这一侧的配对审批与浏览器客户端管理原本打 Blade 后台
     * （`admin.manual-publications.browser-connect.*` / `admin.account.browser-clients.*`），
     * 旧后台删除后改打 React 走的 `/api/v1` 等价端点，鉴权从会话变为 Bearer token。
     * 插件侧（`browser-operations/*`）的协议本来就只走 API，未受影响。
     *
     * @param  list<string>  $scopes
     */
    private function token(Admin $admin, array $scopes): string
    {
        return $admin->createToken('browser-operations-test', $scopes)->plainTextToken;
    }
