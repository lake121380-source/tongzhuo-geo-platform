...
final class AdminUrlImportAiExecutionIdentityTest extends TestCase
{
    ...
    public function test_web_creation_persists_stable_execution_identity_and_requested_model_snapshot(): void
    {
        ...
        $this->withToken($this->token($admin, 'url-identity-write'))
            ->withHeader('X-Idempotency-Key', 'url-identity-create-1')
    ...
    public function test_web_run_refuses_a_permanently_failed_job_without_external_requests(): void
    {
        ...
        $this->withToken($this->token($admin, 'url-web-permanent-write'))
            ->withHeader('X-Idempotency-Key', 'url-web-permanent-run-1')
            ->postJson('/api/v1/url-imports/'.$job->id.'/run')
            ->assertUnprocessable()
            ->assertJsonPath('error.code', 'url_import_not_retryable');

        Http::assertNothingSent();
        $job->refresh();
        $this->assertFalse((bool) $job->retryable_failure);
        $this->assertSame(PermanentAiProviderException::ERROR_CODE, $job->error_code);
    }
    ...
    public function test_web_actor_cannot_run_or_commit_another_admins_job(): void
    {
        ...
        $peerToken = $this->token($peer, 'url-web-job-peer-write');
        $this->withToken($peerToken)
            ->withHeader('X-Idempotency-Key', 'url-peer-run-1')
            ->postJson('/api/v1/url-imports/'.$job->id.'/run')
            ->assertNotFound();
        $this->withToken($peerToken)
            ->withHeader('X-Idempotency-Key', 'url-peer-commit-1')
