 ...
 * AI 工作台的行为契约。
 *
 * **2026-09-12 退役改指**：这份用例原先是打 **Blade 后台的 `/admin/ai-workspace/**` 端点**
 * （`admin.ai-workspace.messages.store` 这套路由名）。旧后台删除后那些路由不存在了，
 * 而 React 后台的 AI 工作台走的是 **`/api/v1/ai-workspace/**`**（见 `ai-ui-src/src/api/geoflowClient.ts`），
 * 两者共用同一套服务（`AdminHelpAnswerStream` / `AiConversationRepository` / 帮助目录），
 * 所以这里的行为断言整体改打真实的 API 端点——**不是删掉覆盖，而是换到还在的那个入口**。
 * 原「Blade 页面渲染」用例连同页面一起删除（React 页面不是这套 HTML 契约的对象）。
 */
final class AdminAiWorkspaceTest extends TestCase
{
    ...
    public function test_conversations_are_owned_renamed_archived_and_return_messages_without_runs(): void
    {
        ...
        $conversationId = $this->withToken($this->token($owner))
            ->withHeader('X-Idempotency-Key', 'conversations-owner-create')
            ->postJson('/api/v1/ai-workspace/conversations', ['title' => '帮助会话'])
            ->assertCreated()
            ->json('data.conversation.id');

        $this->withToken($this->token($other))
            ->getJson('/api/v1/ai-workspace/conversations/'.$conversationId)
            ->assertNotFound();

        $this->withToken($this->token($owner))
            ->withHeader('X-Idempotency-Key', 'conversations-owner-rename')
            ->patchJson('/api/v1/ai-workspace/conversations/'.$conversationId, ['title' => '新的名称'])
            ->assertOk()
            ->assertJsonPath('data.conversation.title', '新的名称');

        $show = $this->withToken($this->token($owner))
            ->getJson('/api/v1/ai-workspace/conversations/'.$conversationId)
            ->assertOk()
            ->assertJsonMissingPath('data.conversation.runs')
            ->assertJsonStructure(['data' => ['conversation' => ['messages', 'message_page']]]);

        self::assertSame([], $show->json('data.conversation.messages'));

        $this->withToken($this->token($owner))
            ->withHeader('X-Idempotency-Key', 'conversations-owner-archive')
            ->postJson('/api/v1/ai-workspace/conversations/'.$conversationId.'/archive')
            ->assertOk()
    ...
    public function test_message_endpoint_streams_status_delta_and_done_then_persists_the_complete_answer(): void
    {
        ...
        $response = $this->withToken($this->token($admin))->postJson(
            $this->messagesUri($conversation),
            ['prompt' => '如何创建一个任务？'],
        );
        ...
        self::assertStringContainsString('/geo_admin?tab=tasks', $messages->last()->meta['related_features'][0]['url']);
    ...
    public function test_first_answer_emits_the_local_title_before_model_text_and_later_questions_keep_it(): void
    {
        ...
        $stream = $this->withToken($this->token($admin))->postJson(
            $this->messagesUri($conversation),
            ['prompt' => $question],
        )->streamedContent();
        ...
        $secondStream = $this->withToken($this->token($admin))->postJson(
            $this->messagesUri($conversation),
            ['prompt' => '第二个问题'],
        )->streamedContent();
        self::assertSame($expected, $conversation->fresh()->title);
    ...
    public function test_low_information_first_question_uses_a_readable_local_fallback_title(): void
    {
        ...
        $this->withToken($this->token($admin))->postJson(
            $this->messagesUri($emojiConversation),
            ['prompt' => '👋👋👋'],
        )->streamedContent();
    ...
    public function test_automatic_title_does_not_replace_an_explicit_or_manually_renamed_title(): void
    {
        ...
        $explicitStream = $this->withToken($this->token($admin))->postJson(
            $this->messagesUri($explicitConversation),
            ['prompt' => '首个问题'],
        )->streamedContent();
    ...
    public function test_stream_timeout_returns_local_help_without_partial_persistence(): void
    {
        ...
        self::assertStringContainsString('event: error', $stream);
        self::assertStringContainsString('ai_unavailable', $stream);
        self::assertStringContainsString(json_encode('/geo_admin?tab=tasks'), $stream);
    ...
    public function test_ai_unavailable_still_returns_local_related_features_and_keeps_the_question(): void
    {
        ...
        self::assertStringContainsString(
            json_encode('/geo_admin?tab=articles'),
            $stream,
        );
        self::assertSame(1, AiConversationMessage::query()->where('conversation_id', $conversation->id)->where('role', 'user')->count());
        self::assertSame(0, AiConversationMessage::query()->where('conversation_id', $conversation->id)->where('role', 'assistant')->count());
    ...
    public function test_input_validation_and_conversation_ownership_are_enforced_before_streaming(): void
    {
        ...
        $url = $this->messagesUri($conversation);

        $this->withToken($this->token($owner))->postJson($url, ['prompt' => ''])->assertUnprocessable();
        $this->withToken($this->token($owner))->postJson($url, ['prompt' => str_repeat('a', 4001)])->assertUnprocessable();
        $this->withToken($this->token($other))->postJson($url, ['prompt' => '查看任务'])->assertNotFound();
    ...
    /**
     * React 后台的 AI 工作台用 Bearer token 访问，会话归属由 token 对应的管理员决定
     * （旧 Blade 端点用的是 `actingAs` 会话）。两者共用同一套服务，所以行为断言不变。
     */
    private function token(Admin $admin): string
    {
        return $admin->createToken('ai-workspace-behaviour-test', ['workspace:read', 'workspace:write'])->plainTextToken;
    }

    private function messagesUri(AiConversation $conversation): string
    {
        return '/api/v1/ai-workspace/conversations/'.$conversation->id.'/messages';
    }
