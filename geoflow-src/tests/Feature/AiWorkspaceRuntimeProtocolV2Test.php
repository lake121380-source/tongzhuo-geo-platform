...
final class AiWorkspaceRuntimeProtocolV2Test extends TestCase
{
    ...
    public function test_history_replays_persisted_features_and_suggestions_without_regeneration(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->getJson('/api/v1/ai-workspace/conversations/'.$conversation->id)
            ->assertOk()
            ->assertJsonPath('data.conversation.messages.1.id', (string) $assistant->id)
            ->assertJsonPath('data.conversation.messages.1.meta.related_features.0.id', 'tasks')
            ->assertJsonCount(3, 'data.conversation.messages.1.meta.suggestions');
    ...
    public function test_message_generation_has_a_separate_per_admin_rate_limit(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->getJson('/api/v1/ai-workspace/conversations')
            ->assertOk();
    }

    /**
     * 2026-09-12 退役改指：AI 工作台的 HTTP 面从 Blade 后台端点迁到 `/api/v1/ai-workspace/**`，
     * 鉴权方式随之从会话（`actingAs`）变为 Bearer token。服务层未变，断言的内容不变。
     */
    private function token(Admin $admin): string
    {
        return $admin->createToken('ai-workspace-protocol-test', ['workspace:read', 'workspace:write'])->plainTextToken;
    }
