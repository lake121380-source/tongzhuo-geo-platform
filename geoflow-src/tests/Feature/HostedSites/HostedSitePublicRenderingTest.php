...
class HostedSitePublicRenderingTest extends TestCase
{
    ...
    public function test_robots_and_sitemap_follow_the_site_indexing_gate(): void
    {
        ...
        $blockedRobots = $this->get('http://alpha.sites.test/robots.txt')->assertOk();
        // 只看 User-agent: * 这一组：按爬虫分组的策略里本来就有正当的 Disallow: /。
        $this->assertContains('Disallow: /', $this->robotsGroup($blockedRobots->getContent(), '*'));
        ...
        $allowedRobots = $this->get('http://alpha.sites.test/robots.txt')->assertOk();
        $this->assertStringContainsString('Sitemap: https://alpha.sites.test/sitemap.xml', $allowedRobots->getContent());
        $starGroup = $this->robotsGroup($allowedRobots->getContent(), '*');
        $this->assertContains('Allow: /', $starGroup);
        $this->assertNotContains('Disallow: /', $starGroup, '允许索引时 User-agent: * 不得整体禁止抓取');
        // 管理路径仍应单独禁止（这正是上面那条不能写成子串匹配的原因）。
        $this->assertContains('Disallow: /geo_admin', $starGroup);
        $this->get('http://alpha.sites.test/sitemap.xml')
            ->assertOk()
    ...
    /**
     * 取 robots.txt 中某个 User-agent 分组内的指令行（分组以空行分隔）。
     *
     * 不能对全文做子串匹配：`Disallow: /geo_admin` 会假命中 `Disallow: /`，
     * 而按爬虫分组的策略（AI 爬虫防线）里本来就有正当的 `Disallow: /`。
     *
     * @return list<string>
     */
    private function robotsGroup(string $content, string $userAgent): array
    {
        $groups = [];
        $current = null;
        foreach (preg_split('/\R/', $content) ?: [] as $line) {
            ...
            if (stripos($line, 'User-agent:') === 0) {
                $current = trim(substr($line, strlen('User-agent:')));
                $groups[$current] ??= [];
