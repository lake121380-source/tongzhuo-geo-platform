...
class HostedSiteDistributionFlowTest extends TestCase
{
    ...
    public function test_article_edit_atomically_rejects_a_duplicate_hosted_fingerprint(): void
    {
        ...
        $token = $admin->createToken('hosted-fingerprint-edit', [
            'articles:write',
            'articles:read',
            'distribution:write',
            'distribution:read',
        ...
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'hosted-fingerprint-article-'.$secondArticle->id)
            ->patchJson('/api/v1/articles/'.$secondArticle->id, [
            ...
            ->assertStatus(422)
            // API 面上这条冲突由 `ArticleGeoFlowService::updateArticle()` 事务末尾的
            // `HostedSiteArticleFingerprintService::synchronizeLockedArticle()` 抛出，
            // 经 `Api/V1/ArticleController` 翻成这个稳定错误码（旧 Blade 面是「回表单 + 会话错误」）。
            ->assertJsonPath('error.code', 'hosted_article_fingerprint_conflict');
        ...
        // 旧 Blade 的「编辑远端文章」页对托管站点渠道会重定向到托管站点详情；
        // React 后台对应的边界由分发记录修正接口承担：托管站点渠道直接拒绝手改远端文章。
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'hosted-fingerprint-job-'.$secondDistribution->id)
            ->patchJson('/api/v1/distribution/jobs/'.$secondDistribution->id, [
                'title' => $secondArticle->title,
                'content' => $secondArticle->content,
            ])
            ->assertStatus(422)
