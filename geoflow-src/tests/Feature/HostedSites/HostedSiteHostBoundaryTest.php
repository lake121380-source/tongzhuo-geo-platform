...
class HostedSiteHostBoundaryTest extends TestCase
{
    ...
    public function test_primary_host_keeps_the_existing_public_and_admin_surfaces(): void
    {
        ...
        // 旧 Blade 后台的 `/geo_admin/login` 已随后台退役删除（React 后台由 nginx 静态直出，
        // 不经过 Laravel）。Laravel 侧仅存的管理端面是 api/v1：主域名必须真正路由它——
        // 未带 token 得到 401，而不是被 host 边界挡成 404。
        $this->getJson('http://primary.test/api/v1/catalog')
            ->assertUnauthorized();
    }
    ...
    public function test_forwarded_host_does_not_override_the_direct_primary_host_without_a_trusted_proxy(): void
    {
        ...
        // 判别器：api/v1 在主域名上被路由（未带 token → 401），在托管域名上被 host 边界挡成 404。
        // 旧 Blade 的 `/geo_admin/login`（主域名 200 / 托管域名 404）已随退役删除。
        $this->withServerVariables([
            'HTTP_X_FORWARDED_HOST' => 'alpha.sites.test',
        ])->getJson('http://primary.test/api/v1/catalog')->assertUnauthorized();
    }
