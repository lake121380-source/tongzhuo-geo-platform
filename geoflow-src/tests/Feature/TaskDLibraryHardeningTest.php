...
class TaskDLibraryHardeningTest extends TestCase
{
    ...
    public function test_url_commit_cleans_database_text_and_controller_never_leaks_the_database_exception(): void
    {
        ...
        $token = $admin->createToken('task-d-commit-failure', ['materials:write'])->plainTextToken;
        ...
        try {
            $response = $this->withToken($token)
                ->withHeader('X-Idempotency-Key', 'task-d-commit-failure-1')
                ->postJson('/api/v1/url-imports/'.$failingJob->id.'/commit');
        } finally {
            DB::statement('DROP TRIGGER IF EXISTS task_d_url_commit_failure');
        }

        // 旧后台用 back()->withErrors 回闪固定文案；API 用固定的 error.code 表达同一个
        // 「导入结果写库失败」，两者都不许把数据库异常原文透给用户。
        $response->assertStatus(422)->assertJsonPath('error.code', 'url_import_commit_failed');
        $this->assertStringNotContainsString($secret, $response->getContent());

        // 【未通过，需要改生产代码】旧后台的 reportCommitFailure() 会把 QueryException
        // 换成只带 SQLSTATE 的 RuntimeException 再上报；api/v1 的 UrlImportController::commit()
        // 直接 report($exception)，于是原始 SQL（含被写入的数据）会进日志。这条断言守住
        // 「上报内容不得携带数据库原文」，留着红是为了不把这条契约悄悄放宽。
        Exceptions::assertNotReported(
            fn (QueryException $exception): bool => str_contains($exception->getMessage(), $secret)
        );
    }
    ...
    public function test_large_import_payloads_are_never_flushed_into_the_session(): void
    {
        ...
        $token = $admin->createToken('task-d-large-import', ['materials:write'])->plainTextToken;
        ...
        // 超限载荷必须被拒（而不是被接受或被回显原文）；API 请求不落 session，
        // 下面同时守住「不得写入 _old_input」这条旧契约。
        $oversizedKeywords = $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-large-keywords-1')
            ->postJson('/api/v1/materials/keyword-libraries/'.$keywordLibrary->id.'/import', ['keywords_text' => $large]);
        $oversizedKeywords->assertStatus(422)->assertJsonPath('error.code', 'library_import_too_large');
        $this->assertStringNotContainsString($large, $oversizedKeywords->getContent());
        $this->assertNull(data_get(session()->all(), '_old_input.keywords_text'));

        $oversizedTitles = $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-large-titles-1')
            ->postJson('/api/v1/materials/title-libraries/'.$titleLibrary->id.'/import', ['titles_text' => $large]);
        $oversizedTitles->assertStatus(422)->assertJsonPath('error.code', 'library_import_too_large');
        $this->assertStringNotContainsString($large, $oversizedTitles->getContent());
        $this->assertNull(data_get(session()->all(), '_old_input.titles_text'));

        $tooManyKeywords = implode("\n", array_fill(0, 1_001, 'duplicate'));
        $tooManyKeywordResponse = $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-large-keywords-2')
            ->postJson('/api/v1/materials/keyword-libraries/'.$keywordLibrary->id.'/import', ['keywords_text' => $tooManyKeywords]);
        $tooManyKeywordResponse->assertStatus(422)->assertJsonPath('error.code', 'library_import_too_many');
        $this->assertNull(data_get(session()->all(), '_old_input.keywords_text'));

        $tooLongTitle = str_repeat('题', 501);
        $tooLongTitleResponse = $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-large-titles-2')
            ->postJson('/api/v1/materials/title-libraries/'.$titleLibrary->id.'/import', ['titles_text' => $tooLongTitle]);
        $tooLongTitleResponse->assertStatus(422)->assertJsonPath('error.code', 'library_import_title_too_long');
        $this->assertNull(data_get(session()->all(), '_old_input.titles_text'));

        $oversizedOutputs = $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-large-url-import-1')
            ->postJson('/api/v1/url-imports', [
                'url' => 'https://example.test',
                'outputs' => array_fill(0, 10_000, 'knowledge'),
            ]);
        $oversizedOutputs->assertStatus(422)
            ->assertJsonPath('error.code', 'validation_failed')
            ->assertJsonPath('error.details.field_errors.outputs', fn ($message): bool => is_string($message) && $message !== '');
    ...
    public function test_oversized_library_descriptions_are_not_flushed_into_the_session(): void
    {
        ...
        $token = $admin->createToken('task-d-large-description', ['materials:write'])->plainTextToken;
        $keywordLibrary = KeywordLibrary::query()->create(['name' => 'Keywords', 'description' => '', 'keyword_count' => 0]);
        $titleLibrary = TitleLibrary::query()->create([
            'name' => 'Titles', 'description' => '', 'title_count' => 0,
            'generation_type' => 'manual', 'generation_rounds' => 1, 'is_ai_generated' => 0,
        ...
        foreach ([
            ['post', '/api/v1/materials/keyword-libraries', 'task-d-large-desc-1'],
            ['patch', '/api/v1/materials/keyword-libraries/'.$keywordLibrary->id, 'task-d-large-desc-2'],
            ['post', '/api/v1/materials/title-libraries', 'task-d-large-desc-3'],
            ['patch', '/api/v1/materials/title-libraries/'.$titleLibrary->id, 'task-d-large-desc-4'],
        ] as [$method, $url, $idempotencyKey]) {
            $response = $this->withToken($token)
            ...
            // 超长描述必须按字段级错误被拒；只有 description 被标记（同请求里的合法 name
            // 不受牵连），且原文既不回显也不落库——旧后台是靠 withInput 的白名单做到的。
            $response->assertStatus(422)
                ->assertJsonPath('error.code', 'validation_failed')
                ->assertJsonPath('error.details.field_errors.description', __('admin.library_validation.field_too_long', [
                    'field' => 'description',
                    'max' => LibraryImportPolicy::DESCRIPTION_MAX_CHARACTERS,
                ]));
            $this->assertSame(['description'], array_keys($response->json('error.details.field_errors')));
            $this->assertStringNotContainsString($oversized, $response->getContent());
            $this->assertNull(data_get(session()->all(), '_old_input.description'));
        }

        $this->assertSame('', (string) $keywordLibrary->fresh()->description);
        $this->assertSame('', (string) $titleLibrary->fresh()->description);
    }
    ...
    public function test_url_normalization_failures_are_reported_with_a_fixed_user_message(): void
    {
        ...
        $token = $admin->createToken('task-d-url-normalization', ['materials:write'])->plainTextToken;

        // 内网地址必须被 SSRF 策略拦下，并且只把固定的 invalid_url 文案交给调用方，
        // 不能把底层异常原文透出去。
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-url-normalization-1')
            ->postJson('/api/v1/url-imports', [
                'url' => 'http://127.0.0.1/private',
                'outputs' => ['knowledge'],
            ])
            ->assertStatus(422)
            ->assertJsonPath('error.code', 'invalid_url')
            ->assertJsonPath('error.message', __('admin.url_import.error.invalid_url'));
    }
    ...
    public function test_web_library_and_entry_forms_reject_internal_nul_and_invalid_utf8(): void
    {
        ...
        $token = $admin->createToken('task-d-storage-policy', ['materials:write'])->plainTextToken;
        $keywordLibrary = KeywordLibrary::query()->create(['name' => 'Keywords', 'description' => '', 'keyword_count' => 0]);
        $titleLibrary = TitleLibrary::query()->create([
            'name' => 'Titles', 'description' => '', 'title_count' => 0,
            'generation_type' => 'manual', 'generation_rounds' => 1, 'is_ai_generated' => 0,
        ...
        // 库名字段带 NUL：只标记 name，同请求里合法的 description 不受牵连。
        // （含非法 UTF-8 的载荷走表单编码：JSON 编码会把非法字节直接变成编码失败。）
        $keywordLibraryResponse = $this->withToken($token)
            ->post('/api/v1/materials/keyword-libraries', [
                'name' => "internal\0nul",
                'description' => 'valid',
            ]);
        $keywordLibraryResponse->assertStatus(422)
            ->assertJsonPath('error.details.field_errors.name', 'name 不能包含 NUL 字符');
        $this->assertSame(['name'], array_keys($keywordLibraryResponse->json('error.details.field_errors')));
        $this->assertNull(data_get(session()->all(), '_old_input.name'));

        $titleLibraryResponse = $this->withToken($token)
            ->patch('/api/v1/materials/title-libraries/'.$titleLibrary->id, [
                'name' => 'valid',
                'description' => "invalid-\xFF-utf8",
            ]);
        $titleLibraryResponse->assertStatus(422)
            ->assertJsonPath('error.details.field_errors.description', 'description 必须是有效的 UTF-8 文本');
        $this->assertSame(['description'], array_keys($titleLibraryResponse->json('error.details.field_errors')));
        $this->assertNull(data_get(session()->all(), '_old_input.description'));
        ...
        $this->withToken($token)
            ...
            ->assertStatus(422)
            ->assertJsonPath('error.details.field_errors.title', '标题不能包含 NUL 字符')
            ->assertJsonPath('error.details.field_errors.keyword', '关联关键词必须是有效的 UTF-8 文本');

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-storage-policy-import-keywords')
            ->post('/api/v1/materials/keyword-libraries/'.$keywordLibrary->id.'/import', [
                'keywords_text' => "valid\ninvalid-\xFF-utf8",
            ])
            ->assertStatus(422)
            ->assertJsonPath('error.code', 'library_import_invalid_utf8');

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-storage-policy-import-titles')
            ->post('/api/v1/materials/title-libraries/'.$titleLibrary->id.'/import', [
    ...
    public function test_boundary_nul_is_trimmed_before_validation_and_saved_as_safe_text(): void
    {
        ...
        $token = $admin->createToken('task-d-boundary-nul', ['materials:write'])->plainTextToken;

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-boundary-nul-1')
        ...
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-boundary-nul-2')
            ->postJson('/api/v1/materials/keyword-libraries/'.$keywordLibrary->id.'/items', [
        ...
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-boundary-nul-3')
            ->postJson('/api/v1/materials/title-libraries', [
        ...
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'task-d-boundary-nul-4')
            ->postJson('/api/v1/materials/title-libraries/'.$titleLibrary->id.'/items', [
    ...
    public function test_task_d_validation_messages_use_the_english_admin_locale(): void
    {
        ...
        $token = $admin->createToken('task-d-english-locale', ['materials:write'])->plainTextToken;
        $englishResponse = $this->withToken($token)
            ->postJson('/api/v1/materials/keyword-libraries', [
                'name' => "internal\0nul",
        ...
        $englishResponse->assertStatus(422)
            ->assertJsonPath('error.code', 'validation_failed')
            ->assertJsonPath('error.message', __('admin.library_validation.validation_failed'))
            ->assertJsonPath(
                'error.details.field_errors.name',
                __('admin.library_validation.field_nul', ['field' => 'name']),
            );
        $this->assertDoesNotMatchRegularExpression('/\p{Han}/u', $englishResponse->getContent());
