...
class TitleGenerationQueueTest extends TestCase
{
    ...
    public function test_title_count_above_the_effective_keyword_count_requires_reuse_confirmation(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->withHeader('X-Idempotency-Key', 'queue-reuse-required')
            ->postJson($this->runsUrl($library), $payload)
            ->assertStatus(422)
    ...
    public function test_title_count_within_the_effective_keyword_count_does_not_require_reuse_confirmation(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->withHeader('X-Idempotency-Key', 'queue-reuse-not-required')
            ->postJson($this->runsUrl($library), $payload)
            ->assertStatus(202);
    ...
    public function test_the_full_keyword_library_count_is_used_for_reuse_confirmation(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->withHeader('X-Idempotency-Key', 'queue-full-keyword-count')
            ->postJson($this->runsUrl($library), $payload)
            ->assertStatus(202);
    ...
    public function test_submitting_the_maximum_creates_one_background_run(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->withHeader('X-Idempotency-Key', 'queue-maximum')
            ->postJson($this->runsUrl($library), $this->validPayload($keywordLibrary, $aiModel, 100_000))
            ->assertStatus(202);
    ...
    public function test_count_above_the_maximum_is_rejected_before_queueing(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->withHeader('X-Idempotency-Key', 'queue-over-maximum')
            ->postJson($this->runsUrl($library), $this->validPayload($keywordLibrary, $aiModel, 100_001))
            ->assertStatus(422)
            ->assertJsonPath('error.code', 'validation_failed')
            ->assertJsonPath(
                'error.details.field_errors.title_count',
                __('admin.title_ai_generate.error.invalid_count', ['max' => 100_000]),
            );
    ...
    public function test_a_large_run_requires_explicit_cost_confirmation(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->withHeader('X-Idempotency-Key', 'queue-large-run-required')
            ->postJson($this->runsUrl($library), $payload)
            ->assertStatus(422)
            ->assertJsonPath('error.code', 'validation_failed')
            ->assertJsonPath(
    ...
    public function test_a_small_run_does_not_require_large_run_confirmation(): void
    {
        ...
        $this->withToken($this->token($admin))
            ->withHeader('X-Idempotency-Key', 'queue-small-run')
            ->postJson($this->runsUrl($library), $payload)
            ->assertStatus(202);
    ...
    public function test_a_library_cannot_start_two_active_runs(): void
    {
        ...
        $this->withToken($token)->withHeader('X-Idempotency-Key', 'queue-two-active-a')
            ->postJson($this->runsUrl($library), $payload)
            ->assertStatus(202);
        $this->withToken($token)->withHeader('X-Idempotency-Key', 'queue-two-active-b')
            ->postJson($this->runsUrl($library), $payload)
            ->assertStatus(409)
    ...
    public function test_status_endpoint_is_scoped_to_the_title_library(): void
    {
        ...
        // 进度投影落在所属标题库的 run 上，且与旧后台轮询端点同构。
        $this->withToken($token)
            ->getJson($this->runsUrl($library).'/'.$run->id)
            ->assertOk()
            ->assertJsonPath('data.run.requested_count', 25)
            ->assertJsonPath('data.run.active', true);
        $this->withToken($token)
            ->getJson($this->runsUrl($library))
            ->assertOk()
            ->assertJsonPath('data.current.id', $run->id);

        // 换到另一个标题库的路径下，同一个 run id 不可见 → 404。
        $this->withToken($token)
            ->getJson($this->runsUrl($otherLibrary).'/'.$run->id)
            ->assertNotFound();
        $this->withToken($token)
            ->getJson($this->runsUrl($otherLibrary))
            ->assertOk()
    ...
    public function test_an_active_run_prevents_its_ai_model_from_being_changed_or_deleted(): void
    {
        [$admin, $keywordLibrary, $library, $aiModel] = $this->createFixtures();
        $run = $this->createRun($library, $keywordLibrary, $aiModel, 5);
        $token = $admin->createToken('queue-models', ['models:read', 'models:write'])->plainTextToken;

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'model-update-while-running')
            ->patchJson('/api/v1/models/'.$aiModel->id, [
            ...
            ->assertStatus(409)
            ->assertJsonPath('error.code', 'model_in_use');
        $this->assertSame('Title Model', $aiModel->fresh()->name);

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'model-delete-while-running')
            ->deleteJson('/api/v1/models/'.$aiModel->id)
            ->assertStatus(409)
            ->assertJsonPath('error.code', 'model_in_use');
        ...
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'model-delete-after-cancel')
            ->deleteJson('/api/v1/models/'.$aiModel->id)
            ->assertOk();
        $run->refresh();
    ...
    public function test_cancel_endpoint_is_scoped_to_the_title_library(): void
    {
        ...
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'queue-cancel-other-library')
            ->postJson($this->runsUrl($otherLibrary).'/'.$run->id.'/cancel')
            ->assertNotFound();

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'queue-cancel-own-library')
            ->postJson($this->runsUrl($library).'/'.$run->id.'/cancel')
            ->assertStatus(202)
    ...
    public function test_detail_prefers_an_active_retried_run_over_a_newer_completed_run(): void
    {
        ...
        // 详情页取「进行中的那一次」——重试后的 run 是 active，优先于更新的已完成 run。
        $this->withToken($this->token($admin))
            ->getJson($this->runsUrl($library))
            ->assertOk()
            ->assertJsonPath('data.current.id', $oldRun->id)
    ...
    {
        return $admin->createToken('title-queue', ['materials:read', 'materials:write'])->plainTextToken;
    }

    private function runsUrl(TitleLibrary $library): string
    {
