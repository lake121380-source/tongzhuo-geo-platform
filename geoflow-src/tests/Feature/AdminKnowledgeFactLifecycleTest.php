...
class AdminKnowledgeFactLifecycleTest extends TestCase
{
    ...
    public function test_terminal_nonretryable_knowledge_fact_history_allows_admin_hard_deletion(): void
    {
        ...
        $token = $superAdmin->createToken('knowledge-fact-hard-delete', ['account:write'])->plainTextToken;

        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'knowledge-fact-hard-delete-'.$executionAdmin->id)
            ->deleteJson('/api/v1/admin/users/'.$executionAdmin->id)
            ->assertOk()
