...
class AdminAiSettingsTest extends TestCase
{
    ...
    public function test_setting_identity_fields_are_the_only_mass_assignable_attributes(): void
    {
        // 这四个身份字段由服务端 AI 设置服务写入，模型为此开了显式白名单（见 AdminAiSetting）。
        // 注意不能再用「是否抛 MassAssignmentException」来验证：显式 $fillable 下 Eloquent
        // 会静默丢弃未列出的键，只有在模型完全 guarded 时才会抛异常。
        // 真正的安全属性是「白名单恰好只有这四个字段、其余一律进不来」。
        $this->assertSame([
            'admin_id',
            'default_chat_model_id',
            'default_embedding_model_id',
            'updated_by_admin_id',
        ], (new AdminAiSetting)->getFillable());
        ...
        $this->assertSame(10, (int) $setting->admin_id);

        // 白名单之外的键不得被写入。
        $setting->fill(['id' => 999, 'created_at' => '2026-01-01 00:00:00', 'injected' => 'x']);
        $this->assertArrayNotHasKey('injected', $setting->getAttributes());
