...
class ApiV1ContractTest extends TestCase
{
    ...
    public function test_logout_revokes_only_the_current_token_and_records_audit(): void
    {
        $admin = $this->createActiveAdmin('logout_admin', 'good-pass');
        $current = $this->createBearerToken($admin, ['catalog:read']);
        $other = $this->createBearerToken($admin, ['catalog:read']);

        $this->withHeader('Authorization', 'Bearer '.$current['plain'])
            ->postJson('/api/v1/auth/logout')
            ->assertOk()
            ->assertJsonPath('data.logged_out', true)
            ->assertJsonPath('data.token_revoked', true);

        $this->withHeader('Authorization', 'Bearer '.$current['plain'])
            ->getJson('/api/v1/catalog')
            ->assertUnauthorized();
        $this->withHeader('Authorization', 'Bearer '.$other['plain'])
            ->getJson('/api/v1/catalog')
            ->assertOk();
