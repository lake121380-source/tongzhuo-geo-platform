...
class AdminAiModelEntryAccessTest extends TestCase
{
    ...
    public function test_admin_batch_start_rejects_a_disabled_persisted_model(): void
    {
        ...
        // 旧后台 `admin.tasks.batch`（兼容接口，只收单任务）在 api/v1 的等价入口是
        // 单任务启动：禁用的持久化模型必须被拒绝，任务保持 paused。
        $this->actingWithToken($actor, ['tasks:write'])
            ->postJson('/api/v1/tasks/'.$task->id.'/start')
            ->assertStatus(409)
            ->assertJsonPath('success', false)
            ->assertJsonPath('error.code', 'ai_model_unavailable')
            ->assertJsonStructure(['error' => ['details' => ['field_errors' => ['ai_model_id']]]]);
    ...
    public function test_admin_toggle_start_rejects_a_persisted_execution_role_change(): void
    {
        ...
        // 旧后台 `admin.tasks.toggle-status` 的「启动」分支在 api/v1 的等价入口是
        // 单任务启动：持久化执行身份失效时必须拒绝，任务保持 paused 且未排期。
        $this->actingWithToken($actor->fresh(), ['tasks:write'])
            ->postJson('/api/v1/tasks/'.$task->id.'/start')
            ->assertStatus(409)
            ->assertJsonPath('success', false)
            ->assertJsonPath('error.code', 'ai_config_access_revoked');
    ...
    public function test_super_admin_task_edit_shows_the_inaccessible_current_model_as_a_sanitized_disabled_option(): void
    {
        ...
        // 旧 Blade 编辑表单把「当前不可访问模型」渲染成 disabled 选项 + hidden input；
        // React 编辑表单改从 api/v1/catalog 取可选项（不含不可访问模型），任务详情把
        // 不可访问模型投影成 null。API 面保留两项可验证行为：
        // ① 详情投影不得泄露模型名/凭据；② 编辑其他字段时，未改动的不可访问模型引用必须原样保留。
        $detail = $this->actingWithToken($editor, ['tasks:read'])
            ...
            ->assertJsonPath('data.ai_quality_model_accessible', false)
            ->assertJsonPath('data.ai_quality_model_access_reason', 'ai_model_not_accessible');
        $detail->assertDontSee('Executor Private Form Model')
            ->assertDontSee('Executor Private Quality Model')
    ...
    public function test_admin_task_form_only_lists_models_the_current_admin_can_use(): void
    {
        ...
        // 旧 Blade 任务创建表单的 aiModels 与 React 侧的 api/v1/catalog models 是同一份
        // 「当前管理员可用模型」投影：自有模型优先、其后是共享模型，越权/系统模型不出现。
        $response = $this->actingWithToken($actor, ['catalog:read'])
            ->getJson('/api/v1/catalog')
            ->assertOk();

        $this->assertSame(
            [$personal->id, $shared->id],
            collect($response->json('data.models'))->pluck('id')->all(),
        );
    }
    ...
    public function test_article_title_and_fact_model_dropdowns_are_scoped_to_the_current_admin(): void
    {
        ...
        // 旧 Blade 的标题生成、知识事实生成、文章创建三处下拉投影的是同一份
        // 「当前管理员可用模型」。React 侧这份投影由 api/v1/catalog 统一提供，
        // 三个入口共用，故对同一投影断言一次。
        $response = $this->actingWithToken($actor, ['catalog:read'])
            ->getJson('/api/v1/catalog')
            ->assertOk();
        ...
        $this->assertStringNotContainsString('legacy-personal', $response->getContent());
    }
    ...
    public function test_secondary_ai_entry_points_reject_a_peer_model_before_dispatch_or_outbound_work(): void
    {
        ...
        // 标题库 AI 生成：越权模型必须在派出任何出站工作之前被拒绝。
        $this->actingWithToken($actor, ['materials:write'])
            ->withHeader('X-Idempotency-Key', 'scoped-title-generation')
            ->postJson("/api/v1/materials/title-libraries/{$titleLibrary->id}/ai-generation-runs", [
        ...
        // 知识事实生成：peer 与 system 模型都必须被拒绝。
        foreach ([$peerModel, $systemModel] as $forbiddenModel) {
            $this->actingWithToken($actor, ['materials:write'])
                ->withHeader('X-Idempotency-Key', 'scoped-fact-generation-'.$forbiddenModel->id)
                ->postJson("/api/v1/materials/knowledge-bases/{$knowledgeBase->id}/fact-generation", [
                    'mode' => 'initial',
                    'target_count' => 1,
        ...
        // 文章编辑器内联生成（旧后台返回 error_code，api/v1 统一为 error.code）。
        $this->actingWithToken($actor, ['articles:write'])
    ...
    public function test_attached_task_optimization_rejects_forged_peer_and_system_models(): void
    {
        ...
        // 旧 Blade 的 admin.articles.ai-quality.optimization.store 已随后台退役删除；
        // React 页面与 API 共用同一条端点，故只保留 API 面断言（peer 与 system 模型均覆盖）。
