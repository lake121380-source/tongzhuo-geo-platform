...
final class AdminAiWorkspaceExecutionIdentityTest extends TestCase
{
     ...
     * 鉴权从会话（`actingAs`）变为 Bearer token。执行身份/权限撤销这些不变量由共享服务承担，
     * 所以断言的内容不变，只换入口。
     */
    private function token(Admin $admin): string
    {
        return $admin->createToken('ai-workspace-execution-identity-test', ['workspace:read', 'workspace:write'])->plainTextToken;
    }
