...
class KnowledgeFactLibraryTest extends TestCase
{
    ...
    public function test_fact_workbench_is_paginated_and_scoped_to_parent_knowledge_base(): void
    {
        ...
        $token = $this->token($admin, 'workbench-read');
        $firstPage = $this->withToken($token)
            ->getJson('/api/v1/materials/knowledge-bases/'.$base->id.'/facts')
            ->assertOk()
            ->assertJsonPath('data.knowledge_base.id', (int) $base->id)
            ->assertJsonPath('data.pagination.total', 26)
            ->assertJsonPath('data.pagination.total_pages', 2);

        $firstItems = json_encode($firstPage->json('data.items'), JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR);
        $this->assertCount(20, (array) $firstPage->json('data.items'));
        $this->assertStringContainsString('指标 1', $firstItems);
        $this->assertStringNotContainsString('其他库秘密指标', $firstItems);

        $secondPage = $this->withToken($token)
            ->getJson('/api/v1/materials/knowledge-bases/'.$base->id.'/facts?page=2')
            ->assertOk()
            ->assertJsonPath('data.pagination.page', 2)
            ->assertJsonPath('data.pagination.total', 26);
        $this->assertCount(6, (array) $secondPage->json('data.items'));
    }
    ...
    public function test_generation_status_exposes_operable_progress_fields(): void
    {
        ...
        $this->withToken($this->token($admin, 'progress-fields-read'))
            ->getJson('/api/v1/materials/knowledge-bases/'.$base->id.'/fact-generation/'.$run->id)
            ->assertOk()
            ->assertJsonPath('data.presented.mode', 'supplement')
            ->assertJsonPath('data.presented.target_count', 20)
            ->assertJsonPath('data.presented.batch.completed', 0)
            ->assertJsonStructure(['data' => ['presented' => ['cancel_url', 'elapsed_seconds', 'actionable_error', 'batch']]]);
    }
    ...
    public function test_admin_can_create_fact_and_stale_lock_version_returns_conflict_without_audit_body_leak(): void
    {
        ...
        $token = $this->token($admin, 'editor-write');
        $response = $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'editor-create-fact-1')
            ->postJson('/api/v1/materials/knowledge-bases/'.$knowledgeBase->id.'/facts', [
        ...
        $this->withToken($token)
            ->withHeader('X-Idempotency-Key', 'editor-stale-update-1')
            ->patchJson('/api/v1/materials/knowledge-bases/'.$knowledgeBase->id.'/facts/'.$factId, [
                'lock_version' => 99, 'label' => '过期更新',
            ])->assertStatus(409)->assertJsonPath('error.code', 'knowledge_fact_revision_conflict');
    ...
    public function test_generation_status_exposes_safe_modal_payload_and_detail_renders_workbench_entry(): void
    {
        ...
        $this->withToken($this->token($admin, 'progress-dialog-read'))
            ->getJson('/api/v1/materials/knowledge-bases/'.$knowledgeBase->id.'/fact-generation/'.$run->id)
            ->assertOk()
            ->assertJsonPath('data.presented.id', $run->id)
            ->assertJsonPath('data.presented.status', 'running')
            ->assertJsonPath('data.presented.active', true)
            ->assertJsonPath('data.presented.candidate_count', 1)
            ->assertJsonPath('data.presented.progress_percent', 15)
            ->assertJsonMissingPath('data.presented.result_json')
            ->assertJsonMissingPath('data.presented.error_message');
        // 旧后台知识库详情页（Blade）渲染的「进入原子事实工作台」入口在 api/v1 上没有等价投影，
        // 原断言随 Blade 页面一并退役。见任务报告。
    }
    ...
    public function test_evidence_created_from_chunk_ignores_spoofed_client_content(): void
    {
        ...
        $this->withToken($this->token($admin, 'evidence-write'))
            ->withHeader('X-Idempotency-Key', 'evidence-spoofed-content-1')
            ->postJson('/api/v1/materials/knowledge-bases/'.$base->id.'/fact-values/'.$value->id.'/evidences', [
    ...
    private function token(Admin $admin, string $name): string
    {
        return $admin->createToken($name, ['materials:read', 'materials:write'])->plainTextToken;
    }
}
