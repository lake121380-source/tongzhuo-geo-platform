<p class="about-lede">
    桐灼GEO 是一套面向 GEO（生成式引擎优化）的开源智能内容工程与多站点分发系统。它把知识库、素材、提示词、AI 生成任务、审核发布、数据分析和多端分发连接成一条可以持续运营的工作流。
</p>
...
<p>
    桐灼GEO 将知识库放在内容生产的起点。团队可以把真实业务资料整理进知识库，再结合标题库、关键词库、图片库、作者和提示词形成生产输入。配置 embedding 模型后，系统会为知识内容建立向量索引，并在文章生成时召回相关资料。
</p>
...
<p>
    桐灼GEO 把内容工程拆成五个连续阶段。每个阶段都能单独管理，也能通过任务、队列和分发渠道形成自动化链路。
</p>
...
<ol class="about-flow">
    ...
    <li>
        ...
        <div>
            ...
            <p>内容可以发布到本地前台，也可以通过 桐灼GEO Agent、WordPress REST 或通用 HTTP API 分发到目标站点。</p>
        </div>
    </li>
    <li>
        <span>05</span>
...
<h2 id="about-capabilities">桐灼GEO 包含的核心能力</h2>
...
<p>
    桐灼GEO 建立在 Laravel 12 与 PHP 8.3+ 之上，使用 PostgreSQL 保存业务数据，并推荐 pgvector 支撑知识向量。Redis 承担队列与缓存，Laravel Scheduler、Horizon 和 Reverb 分别服务于调度、队列运行与实时能力。
</p>
...
<p>
    桐灼GEO 可以作为独立 GEO 官网、现有官网中的内容频道、行业信源站点、内部内容管理后台，或多品牌、多主题的分发中枢。内容团队可以使用素材、任务和审核链路，工程团队可以控制部署、模型和渠道，运营团队可以查看发布、分发和分析结果。
</p>
...
<p>
    桐灼GEO 以 GNU Affero General Public License v3.0 开源发布。个人与组织可以在遵守 AGPL-3.0 的前提下使用、修改、部署和分发；修改后的网络服务应按许可证向其用户提供对应源代码。闭源修改、白标、OEM、商业产品集成或其他需要免除 AGPL-3.0 义务的场景，可向版权所有者申请单独的商业许可。仓库包含应用源码、部署配置、说明文档、测试以及 桐灼GEO Agent Skill。
</p>
