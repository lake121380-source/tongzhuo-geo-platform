 ...
 * 趋势图数值格式化。
 *
 * 后端在「无法测量」时返回 null（例如未声明品牌时的品牌类指标）：必须显示「不可计算」，
 * 不能回落成 0 —— 那会把「无法测量」说成确定结论。真实的 0 仍然照常显示为 0。
 */
export const formatTrendMetricValue = (metric, raw, unavailableLabel, locale) => {
    if (raw === null || raw === undefined || raw === '') return unavailableLabel;
    const number = Number(raw);
    if (!Number.isFinite(number)) return unavailableLabel;
    const decimals = Number.parseInt(metric.decimals || '0', 10);

    return `${number.toLocaleString(locale, { minimumFractionDigits: decimals, maximumFractionDigits: decimals })}${metric.suffix || ''}`;
};
...
const initializeTrendChart = (root) => {
    ...
    // 后端在「无法测量」时返回 null：显示「不可计算」，不回落成 0。
    const unavailableLabel = root.dataset.unavailableLabel || '—';
    ...
    const update = (index, pinned = false) => {
        ...
        metrics.forEach((metric) => {
            ...
            if (value) {
                value.textContent = formatTrendMetricValue(metric, row[metric.key], unavailableLabel, locale);
            }
        });
        root.dataset.pinned = pinned ? 'true' : 'false';
        chart.setAttribute('aria-label', `${row.date || ''}, ${metrics.map((metric) => `${metric.label} ${formatTrendMetricValue(metric, row[metric.key], unavailableLabel, locale)}`).join(', ')}`);
    };
