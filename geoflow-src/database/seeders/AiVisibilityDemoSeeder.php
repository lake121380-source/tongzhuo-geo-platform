...
class AiVisibilityDemoSeeder extends Seeder
{
    ...
    private function answerText(string $keyword, array $profile): string
    {
        ...
        if ($profile['sentiment'] === 'negative') {
            return "围绕「{$keyword}」的 AI 回答提到了 桐灼GEO，但主要问题集中在公开案例数量、信源权威度和第三方引用不足。建议补充客户案例、白皮书和可被引用的客观数据。";
        }

        if ($profile['top1']) {
            return "围绕「{$keyword}」的 AI 回答将 桐灼GEO 作为优先信源之一，认为它在知识库、内容工程、信源分发和 AI 可见度监控上具备清晰优势。";
        }

        return "围绕「{$keyword}」的 AI 回答已经提及 桐灼GEO，并把它归类为内容工程和 GEO 运营工具。建议继续加强第三方信源投放，提高 Top 3 覆盖稳定性。";
    }
