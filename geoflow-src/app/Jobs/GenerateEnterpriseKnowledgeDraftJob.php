...
final class GenerateEnterpriseKnowledgeDraftJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        try {
            ...
            $securityFilteredCount = (int) ($draft['security_filtered_count'] ?? 0);
            if ($securityFilteredCount > 0) {
                $validationItems[] = [
                    'level' => 'danger',
                    'section' => __('admin.enterprise_knowledge.validation.evidence'),
                    'message' => $this->securityFilteredValidationMessage($securityFilteredCount),
                ];
            }
     ...
     * @param  array{content:string,source:string,model_id:?int,error:?string,security_filtered_count?:int,usage_delivery?:EnterpriseKnowledgeDraftUsageDelivery|null}  $draft
     */
    ...
    private function securityFilteredValidationMessage(int $count): string
    {
        $key = 'admin.enterprise_knowledge.error.source_security_filtered';

        return trans()->has($key)
            ? (string) __($key, ['count' => $count])
            : "已隔离 {$count} 份包含疑似提示注入指令的资料，未将其发送给 AI；请人工复核原始资料。";
    }
}
