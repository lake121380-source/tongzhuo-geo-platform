...
final class AdminHelpKnowledgeRetriever
{
    ...
    private function retrieveSnapshot(
        ...
        $relatedPaths = collect($selected)
            ->flatMap(function (array $candidate) use ($admin): array {
                $paths = $this->features->trustedPaths($admin, (string) ($candidate['content'] ?? ''));
                $feature = $this->features->featureForId((string) ($candidate['feature_id'] ?? ''));
                $featurePath = is_array($feature) ? (string) ($feature['path'] ?? '') : '';
                if ($featurePath !== '' && $this->features->canAccessPath($admin, $featurePath)) {
                    $paths[] = $featurePath;
                }
    ...
    private function fallbackCandidates(string $content, string $query, array $matchedFeatures): array
    {
        ...
        $sections = array_values(array_filter(
            $sections,
            fn (array $section): bool => ! $this->securityInspector->hasPromptInjectionRisk([
    ...
    private function sources(
        ...
        return collect($selected)->map(function (array $candidate) use (
            ...
            $paths = $this->features->trustedPaths($admin, (string) ($candidate['content'] ?? ''));
            $feature = $this->features->featureForId((string) ($candidate['feature_id'] ?? ''));
            if (! is_array($feature) || ! $this->features->canAccessPath($admin, (string) ($feature['path'] ?? ''))) {
                $feature = $paths !== [] ? $this->features->featureForPath($paths[0]) : null;
            }
