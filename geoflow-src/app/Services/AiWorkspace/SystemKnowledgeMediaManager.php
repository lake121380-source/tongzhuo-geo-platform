...
final class SystemKnowledgeMediaManager
{
    ...
    public function updateMetadata(KnowledgeMediaAsset $asset, Admin $admin, array $metadata): KnowledgeMediaAsset
    {
        ...
        $this->authorize($knowledgeBase, $admin);
        $tabPath = trim((string) ($metadata['tab_path'] ?? $asset->tab_path));
        if (! $this->features->canAccessPath($admin, $tabPath)) {
            throw new RuntimeException('The selected media tab entry is not a trusted accessible entry.');
        }
    ...
    {
        // 2026-09-12 退役改指：原先这里 `route('admin.ai-workspace.media.show')`——那条 Blade 路由
        // 已随旧后台删除。`route()` 对不存在的路由**会抛 `RouteNotFoundException`**，也就是说
        // 只要回答里带上一张相关截图，整条 SSE 就会 500（不是死链，是崩溃）。
        // React 后台取图走 `/api/v1/ai-workspace/media/{id}`（见 geoflowClient.downloadAiWorkspaceMedia），
        // 路径前缀沿用 `AdminWeb::appPath`，以便部署在 APP_URL 二级目录时同样成立。
        $path = '/api/v1/ai-workspace/media/'.$asset->getKey();
        ...
        return [
            ...
            'url' => AdminWeb::appPath($path),
            'thumbnail_url' => AdminWeb::appPath($path.'?variant=thumbnail'),
    ...
    private function importBytes(
        ...
        $tabPath = trim((string) ($metadata['tab_path'] ?? ''));
        ...
        // 2026-09-12 退役：截图原本挂在一张 Blade 页面的**路由名**上，现在挂的是 React 后台的
        // **页签深链**（`/geo_admin?tab=tasks`）。判据随之从「这条路由是注册过的 GET 页面」
        // 变为「这条深链对应帮助目录里真实存在的一条入口」——`featureForPath` 同时校验了
        // 路径形状与条目存在性。
        if ($this->features->featureForPath($tabPath) === null) {
            throw new RuntimeException('Knowledge media must point at a known admin tab entry.');
        }
