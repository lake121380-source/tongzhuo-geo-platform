...
final readonly class AiWorkspaceCoordinator
{
    ...
    private function answer(
        ...
        $answering = $this->transitionResolutionOwned((string) $run->id, $leaseOwner, 'answering', ['status_message' => '桐灼GEO 正在回答。'], [
    ...
    {
        // 2026-09-12 退役改指：`route_patterns` 现在装的是 React 后台的页签深链
        // （`/geo_admin?tab=…`），不再是 Laravel 路由名——原先的
        // `app('router')->has($route) ? route($route) : route('admin.ai-workspace')`
        // 两边都失效：前者恒假，后者指向已删除的路由、会抛 `RouteNotFoundException`。
        $url = collect($capability->routePatterns)->first(static fn (string $item): bool => str_starts_with($item, '/'))
            ?? AdminTabs::path('ai-workspace');
