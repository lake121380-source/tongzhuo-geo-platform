...
final class SystemKnowledgeBaseManager
{
    ...
    public function update(KnowledgeBase $knowledgeBase, Admin $admin, array $payload): KnowledgeBaseRevision
    {
        ...
        $result = DB::transaction(function () use ($knowledgeBase, $admin, $definition, $content): array {
            ...
            $locked->forceFill([
                ...
                'source_name' => '桐灼GEO official system knowledge',
                'source_type' => 'system',
                'business_line' => '桐灼GEO Admin',
                'risk_level' => 'low',
    ...
    private function validateEditableContent(array $definition, string $content): void
    {
        ...
        // 2026-09-12 退役迁移：站内入口指令由 `[[route:admin.x|标签]]` 改为 `[[tab:<id>|标签]]`。
        // 旧后台的路由名已经不存在；而「这个入口安全吗」的判据也从
        // 「查真实 GET 路由（无参数、非写操作）」变成「页签已知 + 帮助目录里确有该条目的深链」——
        // SPA 里没有「带参数的页」和「非 GET 动作」这些概念，能守的就是这两条。
        $directiveCount = substr_count($content, '[[tab:');
        preg_match_all('/\[\[tab:([a-z0-9_-]+)\|[^\]]+\]\]/u', $content, $matches);
        if ($directiveCount !== count($matches[0] ?? [])) {
            throw new RuntimeException('系统知识中存在格式不完整的 tab 指令。');
        }

        $tabs = array_values(array_unique($matches[1] ?? []));
        foreach ((array) ($definition['required_tabs'] ?? []) as $requiredTab) {
            if (! in_array((string) $requiredTab, $tabs, true)) {
                throw new RuntimeException("系统知识缺少必需入口：{$requiredTab}");
            }
        }

        foreach ($tabs as $tab) {
            $tab = (string) $tab;
            if (! AdminTabs::isKnown($tab)
                || $this->features->featureForPath(AdminTabs::path($tab)) === null) {
                throw new RuntimeException("系统知识包含不安全的 tab 指令：{$tab}");
            }
        }
