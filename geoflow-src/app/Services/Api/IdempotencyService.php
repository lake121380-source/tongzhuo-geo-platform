...
class IdempotencyService
{
     ...
     *
     * 2026-09-12：请求体是**客户端输入**，可能含非法 UTF-8。原先它走 `encodeJson()`，
     * 那里面是 `JSON_THROW_ON_ERROR` —— 于是「带幂等键 + 非法 UTF-8」会返回
     * **500 `idempotency_encode_failed`**，而同一个请求不带幂等键会正常走到 422 字段错误。
     * 这里是纯哈希用途（只比相等），用 `JSON_INVALID_UTF8_SUBSTITUTE` 让非法字节有稳定表示即可；
     * 合法输入的结果与原先逐字节一致，不会让既有幂等键失配。
     *
    ...
    /**
     * Execute a mutation whose side effect is committed by an external
     * system. Unlike executeJson(), this method never removes the reservation
     * after dispatch has started: if the response cannot be persisted, a
     * retry fails closed as in-progress/stale instead of dispatching twice.
     */
    public static function executeExternalJson(Request $request, string $routeKey, Closure $operation): JsonResponse
    {
        $key = self::validatedKey($request);
        if ($key === null || ! in_array($request->method(), ['POST', 'PATCH', 'DELETE'], true)) {
            return $operation();
        ...
        try {
            $requestHash = self::requestHashFor($request, $routeKey);
            $cached = self::loadReplay($key, $storageRouteKey, $requestHash)
                ?? self::loadReplay($key, $routeKey, $requestHash);
            if ($cached !== null) {
                return response()->json($cached['payload'], $cached['status']);
            }
            ...
            try {
                ...
                Log::warning('geoflow.external_idempotency_result_uncertain', [
                    'reservation_id' => $reservation['row_id'],
                    'route_key' => $routeKey,
