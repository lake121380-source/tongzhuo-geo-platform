...
class AiVisibilityAnalyticsService
{
    ...
    public function snapshot(int $days = 60): array
    {
        ...
        $brandBasis = $brandAliases !== [] || $ownedHosts !== [];
        ...
        return [
            'configured' => (bool) ($configuration['configured'] ?? false),
            'kpis' => $this->kpis($this->dailyKeywordMetrics($runs, $brandBasis), $brandBasis),
    ...
    public function overview(int|AiVisibilityAnalyticsFilter $filter = 14): array
    {
        ...
        $brandAliases = $this->brandAliases();
        $ownedHosts = $this->ownedHosts();
        // 品牌归属的可核验依据：声明了品牌名，或声明了自有域名。两者都没有时必须报「不可计算」，
        // 否则恒为 false 的归属判定会把「无法测量」呈现成「品牌可见度为零」这一确定结论。
        $brandBasis = $brandAliases !== [] || $ownedHosts !== [];
        $analyzedRuns = $sampledRuns
            ->map(fn (AiVisibilityRun $run): array => $this->analyzeRun($run, $brandAliases, $ownedHosts, $brandBasis))
            ->values();

        $dailyKeywordMetrics = $this->dailyKeywordMetrics($analyzedRuns, $brandBasis);
        $kpis = $this->kpis($dailyKeywordMetrics, $brandBasis);
        $sourcePreferences = $this->sourcePreferences($analyzedRuns, $brandBasis);
        ...
        return [
            ...
            'brand' => [
                ...
                'names_configured' => $brandAliases !== [],
                'owned_hosts' => $ownedHosts,
                'brand_basis' => $brandBasis,
            ],
            'kpis' => $kpis,
            'polling' => [
                'runs' => $runCount,
            ...
            'latest_runs' => $this->latestRuns($analyzedRuns, $brandBasis),
        ];
    }
     ...
     * 无样本时的 KPI 空态。
     *
     * 三项品牌指标一律为 null：没有样本就没有测量，「可见度 0.0%」是从零数据得出的确定结论。
     * 与品牌归属无关的情感类指标维持原有 0.0（未在本轮口径内）。
     *
     ...
     * @return array<string, float>
     */
    private function kpis(Collection $metricRows, bool $brandBasis): array
    {
        // 无样本时统一走空态：三项品牌指标为 null（没有样本就没有测量）。
        if ($metricRows->isEmpty()) {
            return $this->emptyKpis();
        }

        return [
            'brand_visibility' => $this->averagePercent($metricRows, 'brand_visibility'),
            'top1_rate' => $this->averagePercent($metricRows, 'top1_rate'),
            'top3_rate' => $this->averagePercent($metricRows, 'top3_rate'),
     ...
     * @return array<string, mixed>
     */
    private function analyzeRun(AiVisibilityRun $run, array $brandAliases, array $ownedHosts, bool $brandBasis): array
    {
        $sources = $run->sources
            ->values()
            ->map(function (AiVisibilitySource $source, int $index) use ($brandAliases, $ownedHosts, $brandBasis): array {
                ...
                return [
                    ...
                    'rank' => $rank,
                    // 无可核验依据时归属不可计算，而不是「不是你的信源」。
                    'brand_mentioned' => $brandBasis ? $brandMentioned : null,
                    'text' => $brandText,
                    'term_text' => $termText,
                ];
            });
     ...
     */
    private function dailyKeywordMetrics(Collection $runs, bool $brandBasis): Collection
    {
        return $runs
            ->groupBy(fn (array $run): string => $run['date'].'|'.Str::lower((string) $run['keyword']))
            ->map(function (Collection $rows) use ($brandBasis): array {
                $metric = $this->metricRow($rows, $brandBasis);
                $first = $rows->first();
        ...
        return collect(range(0, $days - 1))
            ->map(function (int $offset) use ($start, $dailyKeywordMetrics): array {
                ...
                return [
                    ...
                    'visibility' => $this->averagePercent($rows, 'brand_visibility'),
                    'top1' => $this->averagePercent($rows, 'top1_rate'),
                    'top3' => $this->averagePercent($rows, 'top3_rate'),
        ...
        return $dailyKeywordMetrics
            ...
            ->map(function (Collection $rows): array {
                return [
                    ...
                    'samples' => (int) $rows->sum('samples'),
                    'brand_visibility' => $this->averagePercent($rows, 'brand_visibility'),
                    'top1_rate' => $this->averagePercent($rows, 'top1_rate'),
                    'top3_rate' => $this->averagePercent($rows, 'top3_rate'),
     ...
     * @return array<string, mixed>
     */
    private function metricRow(Collection $rows, bool $brandBasis): array
    {
        return [
            'samples' => $rows->count(),
            // 这三项都由品牌归属派生（自有域名/品牌名匹配），没有可核验依据时必须报「不可计算」：
            // 恒为 false 会输出 0%，把「无法测量」呈现成「品牌可见度/位次为零」这一确定结论。
            'brand_visibility' => $brandBasis ? $this->percent($rows->where('brand_visible', true)->count(), $rows->count()) : null,
            'top1_rate' => $brandBasis ? $this->percent($rows->where('top1', true)->count(), $rows->count()) : null,
            'top3_rate' => $brandBasis ? $this->percent($rows->where('top3', true)->count(), $rows->count()) : null,
    ...
    /**
     * 求均值并保留「不可计算」：整列为 null 时返回 null，而不是被强转成 0.0。
     *
     * @param  Collection<int, array<string, mixed>>  $rows
     */
    private function averagePercent(Collection $rows, string $key): ?float
    {
        $values = $rows->pluck($key)->reject(fn (mixed $value): bool => $value === null);
     ...
     * @return list<array<string, mixed>>
     */
    private function sourcePreferences(Collection $runs, bool $brandBasis): array
    {
        $sources = [];
        ...
        return collect($sources)
            ->map(function (array $source) use ($brandBasis): array {
                $rankCount = max(1, (int) $source['rank_count']);
                $source['avg_rank'] = round((float) $source['rank_sum'] / $rankCount, 1);
                $source['brand_mentions'] = $brandBasis ? (int) $source['brand_mentions'] : null;
                // 无依据时不给「内容缺口 / 口碑关注 / 强化自有」这类归属结论，也就不会进入关注建议。
                $source['brand_coverage'] = $brandBasis ? $this->percent((int) $source['brand_mentions'], (int) $source['mentions']) : null;
                ...
                $source['action'] = $brandBasis ? $this->sourceAction($source) : 'unavailable';
     ...
     * @return list<array<string, mixed>>
     */
    private function latestRuns(Collection $runs, bool $brandBasis): array
    {
        return $runs
            ...
            ->map(fn (array $run): array => [
                ...
                'date' => $run['date'],
                'brand_visible' => $brandBasis ? $run['brand_visible'] : null,
                'top1' => $brandBasis ? $run['top1'] : null,
                'top3' => $brandBasis ? $run['top3'] : null,
     ...
     */
    /**
     * 品牌名统一走 {@see BrandIdentity}：只认运营方在品牌实体配置里声明的名称。
     * 本站点默认名与产品品牌名都不能当作客户品牌参与匹配。
     *
     * @return list<string>
     */
    ...
    /**
     * 自有域名统一走 {@see BrandIdentity}：只认运营方显式声明的 officialDomain 与已开通 Hosted Site。
     *
     * 本服务同样逐条输出归属（`sources[].brand_mentioned`、`brand.owned_hosts` 都会回传前端），
     * 因此不能像先前那样只看 `geoflow.site_url` / `app.url`——那会把部署默认地址当成「我方域名」，
     * 未做任何声明就报「已配置」，并与查询雷达/竞品雷达对同一条引用给出相反归属。
     *
     * @return list<string>
     */
     ...
     * 百分比指标。
     *
     * **分母为 0 时返回 null（不可计算），不返回 0.0**：零样本下算出来的 0.0 会被读成
     * 「测过了，是零」，而真相是「没有样本可测」——这正是本项目反复强调的那条口径
     * （2026-09-12 拍板统一）。
     */
