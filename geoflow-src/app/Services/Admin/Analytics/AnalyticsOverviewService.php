...
class AnalyticsOverviewService
{
    ...
    public function globalOverview(): array
    {
        ...
        return [
            ...
            // 零样本 → 不可计算（null），不是 0.0%。与 success_rate / conversion_rate 同口径。
            'publish_rate' => $totalArticles > 0 ? round(($publishedArticles * 100) / $totalArticles, 1) : null,
            'ai_generated_articles' => $aiGeneratedArticles,
            'ai_generated_ratio' => $totalArticles > 0 ? round(($aiGeneratedArticles * 100) / $totalArticles, 1) : null,
    ...
    /**
     * @return array{avg_generation_time: float, success_rate: float|null, daily_quota_used: int}
     */
    public function performanceStats(AnalyticsFilter $filter): array
    {
        ...
        return [
            ...
            // 零样本 → 不可计算（null），不是 0.0。
            'success_rate' => $totalFinished > 0 ? round(($completed * 100.0) / $totalFinished, 2) : null,
