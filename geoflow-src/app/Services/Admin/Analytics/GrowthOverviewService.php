...
class GrowthOverviewService
{
    ...
    public function snapshot(bool $includeDistribution): array
    {
        ...
        return [
            'metrics' => [
                ...
                // 品牌类指标在未声明品牌名/自有域名时为 null（不可计算）：保留 null 交给视图显示
                // 「不可计算」，不要强转成 0.0 把「无法测量」呈现成「品牌可见度为零」。
                'brand_visibility_60d' => $ai['kpis']['brand_visibility'] ?? null,
                'new_leads' => $leads['new'],
            ...
            'cards' => [
                ...
                'traffic' => $traffic,
                'ai_visibility' => [
                    'visibility' => $ai['kpis']['brand_visibility'] ?? null,
                    'top3' => $ai['kpis']['top3_rate'] ?? null,
     ...
     * 「下一步该做什么」的告警条。
     *
     * **2026-09-12 退役改指**：原先五个 `href` 都是 `route('admin.*')`——那批 Blade 路由已随旧后台删除，
     * `route()` 对不存在的路由**抛 `RouteNotFoundException`**，所以这条告警条的每一次求值都会 500。
     * 现在指向 React 后台的页签深链（`AdminTabs::path()` 是后端侧的权威页签清单）。
     *
    ...
    {
        if ($leads['new'] > 0) {
            return ['type' => 'new_leads', 'count' => $leads['new'], 'href' => AdminTabs::path('leads')];
        }

        if ($includeDistribution && ($distribution['failed'] ?? 0) > 0) {
            return ['type' => 'distribution_failed', 'count' => $distribution['failed'], 'href' => AdminTabs::path('distribution')];
        }

        if ($failedTasks > 0) {
            return ['type' => 'content_failed', 'count' => $failedTasks, 'href' => AdminTabs::path('tasks')];
        }

        if ($leads['active_forms'] === 0) {
            return ['type' => 'no_forms', 'count' => 0, 'href' => AdminTabs::path('leads')];
        }

        if (! $aiConfigured) {
            return ['type' => 'ai_unconfigured', 'count' => 0, 'href' => AdminTabs::path('ai-models')];
        }
