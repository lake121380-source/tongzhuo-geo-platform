...
class KnowledgeFactAiGenerator
{
    ...
    public function generate(
        ...
        // Keep this as a second boundary in case a caller bypasses the
        // coordinator's chunk hydration path.  Never invoke a model with an
        // empty evidence set: it cannot produce grounded facts and would
        // otherwise consume quota or be misclassified as provider failure.
        $evidence = array_values(array_filter(
            $evidence,
            fn (mixed $row): bool => is_array($row)
                && ! $this->securityInspector->hasPromptInjectionRisk($row),
        ));
        if ($evidence === []) {
            return [];
        }
