...
class KnowledgeFactGenerationCoordinator
{
    ...
    public function processClaimedBatch(
        ...
        foreach ($evidence as $descriptor) {
            if (! is_array($descriptor)
                || preg_match('/\A\d+\z/', (string) ($descriptor['chunk_id'] ?? '')) !== 1
                || preg_match('/\A[a-f0-9]{64}\z/', (string) ($descriptor['content_hash'] ?? '')) !== 1) {
                $this->markObsolete($context);

                return;
            }
            $chunkIds[] = (int) $descriptor['chunk_id'];
        }
        ...
        $hydratedEvidence = [];
        $securityFilteredCount = 0;
        $canonicalDescriptors = [];
        foreach ($evidence as $descriptor) {
            $chunk = $chunks->get((int) $descriptor['chunk_id']);
            $expectedEvidenceKey = $chunk instanceof KnowledgeChunk
                ? 'chunk:'.$chunk->id.':'.substr((string) $chunk->content_hash, 0, 12)
                : '';
            if (! $chunk
                || ! hash_equals((string) $descriptor['content_hash'], (string) $chunk->content_hash)
                || $expectedEvidenceKey === ''
                || ! hash_equals($expectedEvidenceKey, (string) ($descriptor['evidence_key'] ?? ''))) {
                $this->markObsolete($context);

                return;
            }
            // Rebuild the descriptor from server-owned values.  Queue
            // payloads are not trusted input; spreading arbitrary descriptor
            // keys would let a forged job smuggle extra prompt text.
            $canonicalDescriptors[] = [
                'evidence_key' => $expectedEvidenceKey,
            ...
            if ($this->securityInspector->hasPromptInjectionRisk($hydrated)) {
        ...
        $computedInputHash = hash(
            'sha256',
            json_encode($canonicalDescriptors, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE),
        );
        if (! hash_equals($inputHash, $computedInputHash)) {
            $this->markObsolete($context);

            return;
        }
        if ($hydratedEvidence === []) {
            $this->completeBatchWithoutGeneration(
                $context,
                $inputHash,
                count($evidence),
                $securityFilteredCount,
        ...
        foreach ($candidates as $candidateIndex => $candidate) {
            ...
            try {
                ...
                // A custom/alternate generator may apply a stricter second
                // filter and return without invoking the persistence hook.
                // Close the claim explicitly instead of turning that safe
                // empty result into a misleading model-unavailable failure.
                if ($facts === [] && $persistedModel === null) {
                    $this->completeBatchWithoutGeneration(
                        $context,
                        $inputHash,
                        count($evidence),
    ...
    private function finalizeTransaction(
        ...
        DB::transaction(function () use ($runId, $executionAttempt, $leaseToken): void {
            ...
            $noSafeEvidence = $status === KnowledgeFactGenerationRun::STATUS_FAILED
                && $failedBatches === 0
                && $batches !== []
                && collect($batches)->every(static fn (mixed $batch): bool => is_array($batch)
                    && (string) ($batch['status'] ?? '') === 'completed'
                    && (int) ($batch['evidence_count'] ?? 0) > 0
                    && (int) ($batch['security_filtered_count'] ?? 0) >= (int) ($batch['evidence_count'] ?? 0));
            $updates = [
                'status' => $status,
                'active_key' => null,
                'result_json' => $result,
            ...
            if ($noSafeEvidence) {
                $updates['error_code'] = 'knowledge_fact_generation_no_safe_evidence';
                $updates['error_message'] = $this->errorSanitizer->sanitize('knowledge_fact_generation_no_safe_evidence');
            }
            $run->forceFill($updates)->save();
            if ($created > 0) {
    ...
    /**
     * Close a claimed batch when no trustworthy evidence remains after
     * security filtering.  This deliberately avoids an AI invocation: an
     * empty prompt would either waste quota or be interpreted as a model
     * outage, and retrying cannot make quarantined source text safe.
     */
    private function completeBatchWithoutGeneration(
        KnowledgeFactGenerationExecutionContext $context,
        string $inputHash,
        int $evidenceCount,
        int $securityFilteredCount,
    ): void {
        DB::transaction(function () use ($context, $inputHash, $evidenceCount, $securityFilteredCount): void {
            $this->executionGuard->assertCurrent($context);
            $run = KnowledgeFactGenerationRun::query()->whereKey($context->runId)->lockForUpdate()->firstOrFail();
            if (! $run->isActive()) {
                return;
            ...
            $batchResult = [
                ...
                'completion_reason' => $securityFilteredCount > 0
                    ? 'prompt_injection_quarantined'
                    : 'no_evidence',
            ];
            $result['batches'][(string) $context->batchSequence] = $batchResult;
            $claim['status'] = 'completed';
            $claim['candidate_count'] = 0;
            $claim['evidence_count'] = $evidenceCount;
            $claim['security_filtered_count'] = $securityFilteredCount;
            $claim['completion_reason'] = $batchResult['completion_reason'];
            $claim['lease_token'] = null;
            $claim['lease_expires_at'] = null;
