...
{
    /** 旧后台作者详情页固定陈列 10 篇（`Admin\AuthorController::detail`）。 */
    public const AUTHOR_ARTICLE_LIMIT = 10;

    private const AUTHOR_ARTICLE_LIMIT_MAX = 100;
    ...
    public function show(string $type, int $id): array
    {
        ...
        return [
            ...
            'usage_total' => $this->usageTotal($type, $id),
        ];
    }

    /**
     * 素材库的「被使用量」——三种库各有各的旧后台口径，逐条对齐，不统一成一种算法：
     * 关键词库是**引用了本库任一关键词的文章数**（`articles.original_keyword`）
     * （`Admin\KeywordLibraryController::loadUsageTotal`）；
     * 标题库是本库所有标题的 `used_count` 之和（`Admin\TitleLibraryController::detail`）；
     * 图片库是**引用了本库图片的文章图片关系数**（`Admin\ImageLibraryController::detail`）。
     *
     * 其余类型旧后台没有这个统计，返回 null，不要用 0 冒充「统计过了，是零」。
     * 只在详情（{@see self::show}）里算，列表里算会变成每行一查。
     */
    private function usageTotal(string $type, int $id): ?int
    {
        return match ($type) {
            'keyword-libraries' => $this->keywordLibraryUsageTotal($id),
            'title-libraries' => (int) (Title::query()->where('library_id', $id)->sum('used_count') ?? 0),
            'image-libraries' => (int) ArticleImage::query()
                ->whereHas('image', static fn (Builder $query) => $query->where('library_id', $id))
                ->count(),
            default => null,
        };
    }

    private function keywordLibraryUsageTotal(int $libraryId): int
    {
     ...
     * @return array<string,mixed>
     */
    public function listItems(string $type, int $parentId, int $page = 1, int $perPage = 20, string $search = ''): array
    {
        $type = $this->normalizeItemType($type);
        $this->findMaterial($type, $parentId);
        $query = $this->buildItemQuery($type, $parentId);
        $this->applyItemSearch($query, $type, trim($search));
     ...
     * 条目级搜索。
     *
     * 只有图片库在旧后台有对应的搜索框（`Admin\ImageLibraryController::loadDetailImages`
     * 按 original_name / filename / file_name 匹配）。关键词库的搜索框在旧后台是
     * 另一套实现、标题库详情页根本没有搜索——不要在这里凭空给它们发明匹配字段，
     * 否则「后台搜不到、API 搜得到」会变成新的不一致。
     */
    private function applyItemSearch(Builder $query, string $type, string $search): void
    {
        if ($search === '' || $type !== 'image-libraries') {
            return;
        }
     ...
     * 一次上传多张图片。
     *
     * 旧后台的语义是**逐张独立**：某一张失败不影响其余，失败的记下来继续处理下一张
     * （`Admin\ImageLibraryController::uploadImages`）。这里保留同一语义。
     *
     * 两个入口共用这个方法，因为「失败一张要不要回滚整批」「补偿掉的临时文件谁来删」
     * 这类判断一旦各写一遍，迟早会分叉成「后台跳过、API 整批失败」。
     *
     * 张数上限交给 PHP 的 `max_file_uploads`（与旧后台上传页同一个上限），不在这里
     * 另造一个数字。`errors` 里的原文案只给旧后台的提示用，API 侧不要透出——
     * 盘符、临时路径这类信息不该进响应；原始原因进日志。
     *
     * @param  array<int, UploadedFile>  $images
     * @return array{uploaded:int,skipped:int,total:int,errors:list<array{original_name:string,message:string}>}
     */
    public function createUploadedImageItems(string $type, int $parentId, array $images): array
    {
        $type = $this->normalizeWritableItemType($type);
        if ($type !== 'image-libraries') {
            $this->validationError('images', '仅图片库支持图片上传');
        }
        $this->findMaterial($type, $parentId);
        ...
        foreach ($images as $image) {
            ...
            try {
                $this->withUploadedImagePathLock(
                    $type,
                    $parentId,
                    $image,
                    function () use ($type, $parentId, $image): void {
                        $this->createUploadedImageItem($type, $parentId, $image);
                    },
                );
                $uploaded++;
            } catch (\Throwable $exception) {
     ...
     * 某作者最近的几篇文章。
     *
     * 对齐旧后台作者详情页（`Admin\AuthorController::detail`）：按创建时间倒序取 10 条，
     * 字段就是页面上列出的那几个。旧页虽然 select 了 `deleted_at`，但 Article 有软删除
     * 全局作用域、并没有调 `withTrashed()`，所以**回收站里的文章不在其中**——这里保持同一结果。
     *
     * @return array{author_id:int,articles:list<array<string,mixed>>}
     */
    public function recentArticlesByAuthor(int $authorId, int $limit = self::AUTHOR_ARTICLE_LIMIT): array
    {
        $this->findMaterial('authors', $authorId);
        $limit = max(1, min($limit, self::AUTHOR_ARTICLE_LIMIT_MAX));
        ...
        return [
            'author_id' => $authorId,
            'articles' => $articles->map(static fn (Article $article): array => [
                'id' => (int) $article->id,
    ...
    private function normalizeKnowledgePayload(array $data, bool $isUpdate): array
    {
        ...
        // Keep the metadata fields used by the original knowledge-base form
        // available to the JSON adapter as well.  They are persisted on the
        // same knowledge_bases row and are intentionally normalized here so
        // React and the legacy form share one validation boundary.
        foreach ([
            'source_name' => 255,
            'source_url' => 500,
            'source_type' => 80,
            ...
            if (array_key_exists($field, $data) || ! $isUpdate) {
                $payload[$field] = $this->optionalString($data, $field, $maxLength);
            }
        }
        // effective_date 落库是 date 列，不能当普通字符串处理：空串会让 PostgreSQL
        // 直接抛 invalid input syntax for type date，必须归一成 null 并校验格式。
        if (array_key_exists('effective_date', $data) || ! $isUpdate) {
            $payload['effective_date'] = $this->optionalDate($data, 'effective_date');
        }
        if (array_key_exists('risk_level', $data) || ! $isUpdate) {
            $riskLevel = trim((string) ($data['risk_level'] ?? 'medium'));
            if (! in_array($riskLevel, ['low', 'medium', 'high'], true)) {
                $this->validationError('risk_level', '风险等级无效');
            }
            $payload['risk_level'] = $riskLevel;
        }
        if (array_key_exists('review_status', $data) || ! $isUpdate) {
            $reviewStatus = trim((string) ($data['review_status'] ?? 'unreviewed'));
            if (! in_array($reviewStatus, ['unreviewed', 'reviewing', 'reviewed', 'rejected'], true)) {
                $this->validationError('review_status', '审核状态无效');
            }
     ...
     * 日期字段的归一化：空值统一成 null。
     *
     * PostgreSQL 的 date 列不接受空字符串（invalid input syntax for type date），
     * 而 SQLite 会默默接受，所以这类问题只在真实 PostgreSQL 上暴露。
     *
     * @param  array<string,mixed>  $data
     */
    private function optionalDate(array $data, string $field): ?string
    {
        ...
        if ($parsed === false || $parsed->format('Y-m-d') !== $value) {
            $this->validationError($field, '生效日期格式应为 YYYY-MM-DD');
        }
