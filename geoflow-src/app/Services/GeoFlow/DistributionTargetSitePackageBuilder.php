...
HTACCESS;
    ...
    private function initialStaticIndex(DistributionChannel $channel): string
    {
        ...
        return $head
            ...
            .$experienceHtml
            .'<div class="empty">暂无文章。请先从 桐灼GEO 发布一篇绑定此渠道的文章。</div></main>'
...
{
    $siteName = trim((string) ($settings['site_name'] ?? $config['site_name'] ?? '桐灼GEO Target Site'));
    $siteName = $siteName !== '' ? $siteName : '桐灼GEO Target Site';
    ...
    return [
        ...
        'site_description' => trim((string) ($settings['site_description'] ?? $config['site_description'] ?? '由 桐灼GEO 自动分发和管理的目标站点。')),
...
function renderHomePage(array $config): void
{
    ...
    if ($articles === []) {
        echo '<div class="card empty">暂无文章。请先从 桐灼GEO 发布一篇绑定此渠道的文章。</div>';
        pageFooter($config);
        return;
    }
...
function renderArticlePage(array $config, string $slug): void
{
    ...
    echo jsonLdScript([
        ...
        "author"=>[
            "@type"=>"Person",
            "name"=>is_array($article['author'] ?? null) ? (string) ($article['author']['name'] ?? '桐灼GEO') : '桐灼GEO',
        ],
        "publisher"=>[
            "@type"=>"Organization",
...
function renderLlmsText(array $config): string
{
    ...
    $lines = [
        '# '.($siteName !== '' ? $siteName : '桐灼GEO Target Site'),
        '',
    ];
    if ($description !== '') {
        $lines[] = '> '.$description;
