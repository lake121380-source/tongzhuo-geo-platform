...
final class ArticleAiQualityScorerV2
{
    ...
    public function score(array $modelResult, int $passScore = 85, int $manualOverrideMinScore = 70): array
    {
        ...
        foreach ($issues as &$issue) {
            // 白名单外的 code：不能崩（整次质检作废等于门禁永久失效），也不能静默放过——
            // 记为需人工复核的门禁理由，同时照常按它自报的严重度参与判定（fail-closed）。
            if (($issue['code_recognized'] ?? true) === false) {
                $gateReasons[] = 'unrecognized_issue_code';
            }
    ...
    private function groupIssues(array $issues): array
    {
        ...
        foreach ($issues as $issue) {
            ...
            // 与 V1 一致：白名单外的 code 只打标不抛异常，避免一次格式偏差让整次质检作废。
