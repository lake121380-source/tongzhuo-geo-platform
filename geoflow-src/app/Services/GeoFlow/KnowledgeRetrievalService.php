...
class KnowledgeRetrievalService
{
    ...
    public function __construct(
        ...
        private readonly KnowledgeEvidenceSecurityInspector $securityInspector,
    ) {}
    ...
    public function retrieveContextBundleFromMany(
        ...
        $retrievalRequestId ??= (string) Str::uuid();
        $retrieval = $this->retrieveEvidenceFromManyWithMetadata(
            $knowledgeBaseIds,
            $query,
            max($limit * 4, 16),
            identity: $identity,
        ...
        return [
            ...
            'evidence' => $evidence,
            'retrieval_meta' => $evidence !== []
                ? $this->retrievalMetadata($evidence)
    ...
    public function retrieveEvidenceFromMany(
        ...
        return $this->retrieveEvidenceFromManyWithMetadata(
            $knowledgeBaseIds,
            $query,
            $candidateLimit,
    ...
    /**
     * @param  list<int>  $knowledgeBaseIds
     * @return array{evidence:list<array<string,mixed>>,retrieval_meta:array<string,mixed>}
     */
    private function retrieveEvidenceFromManyWithMetadata(
        array $knowledgeBaseIds,
        string $query,
        int $candidateLimit = 16,
        bool $allowRemoteEmbedding = true,
        ...
        if ($knowledgeBaseIds === []) {
            return [
                'evidence' => [],
                'retrieval_meta' => KnowledgeQueryEmbeddingResult::incompatible('no_evidence')->safeMetadata(),
            ];
        }

        if (count($knowledgeBaseIds) === 1) {
            return $this->retrieveEvidenceWithMetadata(
                $knowledgeBaseIds[0],
                $query,
                $candidateLimit,
                $allowRemoteEmbedding,
        ...
        $merged = [];
        $retrievalMetadata = KnowledgeQueryEmbeddingResult::incompatible('no_evidence')->safeMetadata();
        $hasRetrievalMetadata = false;

        foreach ($knowledgeBaseIds as $order => $knowledgeBaseId) {
            $retrieval = $this->retrieveEvidenceWithMetadata(
                $knowledgeBaseId,
                $query,
                $perBaseLimit,
                $allowRemoteEmbedding,
            ...
            if (! $hasRetrievalMetadata || $retrieval['retrieval_meta']['reason'] !== 'no_evidence') {
                $retrievalMetadata = $retrieval['retrieval_meta'];
                $hasRetrievalMetadata = true;
            }
            foreach ($retrieval['evidence'] as $candidate) {
        ...
        return [
            'evidence' => array_slice($merged, 0, max(1, $candidateLimit)),
            'retrieval_meta' => $retrievalMetadata,
        ];
    }
     ...
     * @return array{evidence:list<array<string,mixed>>,retrieval_meta:array<string,mixed>}
     */
    private function retrieveEvidenceWithMetadata(
        int $knowledgeBaseId,
        string $query,
        int $candidateLimit = 16,
        bool $allowRemoteEmbedding = true,
        ...
        if ($rows === []) {
            return [
                'evidence' => [],
                'retrieval_meta' => $this->retrievalMetadataForResult($embeddingResult, false),
            ];
        }
        ...
        foreach ($rows as $row) {
            ...
            if ($this->securityInspector->hasPromptInjectionRisk([
                'content' => $content,
                'chunk_title' => $title,
            ...
            // Metadata describes authority, but it is not topical relevance.
            // Do not let a reviewed/source-rich chunk with zero lexical and
            // vector overlap enter the RAG context merely because the base is
            // small enough for a full scan.  Large bases already get this
            // behavior from their lexical prefilter; keep both paths aligned.
            if ($vectorScore <= 0.0 && $lexicalScore <= 0.0 && $titleScore <= 0.0) {
                continue;
            }
        ...
        return [
            'evidence' => array_slice($scored, 0, max(1, $candidateLimit)),
            'retrieval_meta' => $this->retrievalMetadataForResult($embeddingResult, $scored !== []),
        ];
    }
    ...
    /**
     * Keep an embedding failure reason visible even when relevance/governance
     * filtering leaves no evidence. A successful embedding with no candidates
     * remains the ordinary `no_evidence` outcome.
     *
     * @return array<string,mixed>
     */
    private function retrievalMetadataForResult(KnowledgeQueryEmbeddingResult $result, bool $hasEvidence): array
    {
        return $hasEvidence || ! $result->successful()
            ? $result->safeMetadata()
