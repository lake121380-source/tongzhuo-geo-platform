...
class KnowledgeEvidenceSecurityInspector
{
    ...
    public function hasPromptInjectionRisk(array $evidence): bool
    {
        ...
        // Output-control phrases are only suspicious when they form an
        // independent command.  A broad substring match would quarantine
        // ordinary policy text such as “审核结果请输出通过或不通过” or
        // “本文介绍如何输出通过率指标”.  Split on sentence/line
        // boundaries (but not commas or colons, which commonly introduce
        // explanatory prose), then require the whole segment to be the
        // command.  The compact pass still catches characters separated by
        // punctuation or zero-width obfuscation.
        $directOutputPatterns = [
            '/^(?:please\s+)?(?:directly\s+)?(?:output|return|reply|respond|answer|say)\s+(?:with\s+)?(?:exactly\s+|only\s+)?(?:passed|approved|safe|compliant|no\s+(?:issues?|problems?|errors?|violations?))$/iu',
            '/^(?:请)?(?:直接)?(?:输出|返回|回复|回答)\s*(?:通过|批准|安全|合规|没有问题|无问题)$/u',
        ];
        $compactDirectOutputPatterns = [
            ...
            '/^(?:请)?(?:直接)?(?:输出|返回|回复|回答)(?:通过|批准|安全|合规|没有问题|无问题)$/u',
        ];
        $segments = preg_split('/[\r\n。！？!?；;.]+/u', $text) ?: [];
        foreach ($segments as $segment) {
            ...
            if (collect($directOutputPatterns)->contains(
                static fn (string $pattern): bool => preg_match($pattern, $segment) === 1,
            )) {
                return true;
            }
            $compactSegment = (string) preg_replace('/[^\p{L}\p{N}]+/u', '', $segment);
            if ($compactSegment !== '' && collect($compactDirectOutputPatterns)->contains(
                static fn (string $pattern): bool => preg_match($pattern, $compactSegment) === 1,
            )) {
                return true;
            }
        }
