...
class UrlImportProcessingService
{
    ...
    /**
     * Fetch and parse a public page using the same SSRF-safe transport and
     * extraction rules as URL-to-knowledge imports.  The scanner uses this
     * read-only projection and never creates an import job or invokes a model.
     *
     * @return array{url:string,host:string,status:int,html:string,page:array{title:string,description:string,text:string,summary:string,raw_json:array<string,mixed>}}
     */
    public function inspectPage(string $input): array
    {
        $normalized = $this->normalizeInputUrl($input);
        $fetched = $this->fetchPage($normalized['url']);
        ...
        return [
            ...
            'page' => $this->parseHtml((string) $fetched['html'], $normalized['url']),
        ];
    }
    ...
    public function process(UrlImportJob $job): UrlImportJob
    {
        ...
        try {
            ...
            // Keep the requested output set alongside the immutable preview.
            // Older jobs have no option recorded and intentionally retain the
            // historical all-output behaviour.
            $analysis['requested_outputs'] = $this->requestedOutputs($job);
    ...
    public function commit(UrlImportJob $job): array
    {
        ...
        $outputs = $this->requestedOutputs($job);
        $includeKnowledge = in_array('knowledge', $outputs, true);
        $includeKeywords = in_array('keywords', $outputs, true);
        $includeTitles = in_array('titles', $outputs, true);
        $knowledgeContent = trim(LibraryImportPolicy::sanitizeDatabaseText(
            (string) ($analysis['knowledge_markdown'] ?? $page['text'] ?? ''),
        ));
        if ($includeKnowledge && $knowledgeContent === '') {
            throw new \RuntimeException(__('admin.url_import.error.commit_before_parse'));
        }
        ...
        if ($includeTitles && $titles === []) {
            throw new \RuntimeException(__('admin.url_import.error.ai_titles_missing'));
        }

        $summary = DB::transaction(function () use ($job, $expectedResultHash, $baseName, $knowledgeContent, $knowledgeDescription, $keywords, $titles, $includeKnowledge, $includeKeywords, $includeTitles): array {
            ...
            $summary = [
                'outputs' => array_values(array_filter([
                    $includeKnowledge ? 'knowledge' : null,
                    $includeKeywords ? 'keywords' : null,
                    $includeTitles ? 'titles' : null,
                ])),
                'knowledge_base' => $knowledgeBase?->id !== null ? (int) $knowledgeBase->id : null,
                'keyword_library' => $keywordLibrary?->id !== null ? (int) $keywordLibrary->id : null,
                'title_library' => $titleLibrary?->id !== null ? (int) $titleLibrary->id : null,
                'keywords' => $keywordLibrary?->id !== null
                    ? (int) Keyword::query()->where('library_id', (int) $keywordLibrary->id)->count()
                    : 0,
                'titles' => $titleLibrary?->id !== null
                    ? (int) Title::query()->where('library_id', (int) $titleLibrary->id)->count()
                    : 0,
            ];
    ...
    /**
     * Return the output libraries requested when the job was created.
     *
     * A missing/invalid option is treated as the historical all-output mode so
     * jobs created by the Blade importer remain backwards compatible.  The
     * list is kept server-side and is never trusted from a commit request.
     *
     * @return list<'knowledge'|'keywords'|'titles'>
     */
    private function requestedOutputs(UrlImportJob $job): array
    {
        $options = json_decode((string) $job->options_json, true);
        $raw = is_array($options) && is_array($options['outputs'] ?? null)
            ? $options['outputs']
            : [];
        $allowed = ['knowledge', 'keywords', 'titles'];
        ...
        return $outputs === [] ? $allowed : $outputs;
    }
    ...
    private function fetchPage(string $url): array
    {
        $request = $this->http->timeout(20)
            ->connectTimeout(8)
            ->withHeaders([
                'User-Agent' => '桐灼GEO URL Importer/1.0',
        ...
        foreach ($models as $candidateIndex => $model) {
            for ($attempt = 1; $attempt <= self::AI_ANALYSIS_MAX_ATTEMPTS; $attempt++) {
                ...
                try {
                    ...
                    if ($aiKnowledge === '' && $includeKnowledge) {
                        throw new \RuntimeException(__('admin.url_import.error.ai_knowledge_missing'));
                    }
                    if ($aiKnowledge === '') {
                        // Keyword/title-only imports still need context for
                        // their prompts, but do not require a knowledge
                        // artifact from the provider.
                        $aiKnowledge = trim((string) ($cleaned['text'] ?? $text));
                    }
                    ...
                    $aiKeywords = [];
                    if ($includeKeywords || $includeTitles) {
    ...
    {
        return <<<'PROMPT'
你是 桐灼GEO 的网页正文清洗器。只输出 JSON，不要输出 Markdown 代码块。
...
PROMPT;
    ...
    {
        return <<<'PROMPT'
你是 桐灼GEO 的核心业务关键词提炼器。只输出 JSON，不要输出 Markdown 代码块。
字段固定为：keywords。
...
PROMPT;
    ...
    {
        return <<<'PROMPT'
你是 桐灼GEO 的 GEO 标题库构建器。只输出 JSON，不要输出 Markdown 代码块。
字段固定为：titles。
...
PROMPT;
    ...
    {
        return "请为 桐灼GEO 标题库生成 50 个可用于内容任务的标题。标题要围绕核心业务词展开，必须服务于用户在 AI 搜索中的真实问题、比较、选型、采购、实施或运营决策。\n\n"
    ...
    {
        return <<<'PROMPT'
你是 桐灼GEO 的知识库构建器。只输出 JSON，不要输出 Markdown 代码块。
...
PROMPT;
    ...
    {
        return "请基于页面 JSON 和清洗正文生成可直接入库的 桐灼GEO 知识库 Markdown。先识别核心业务，再把页面信息拆成结构化、原子化事实，最后归纳 GEO 内容可用方向和使用边界。\n\n"
    ...
    {
        return <<<'PROMPT'
你正在为 桐灼GEO 构建可复用素材库。请把网页内容拆成三类资产：
