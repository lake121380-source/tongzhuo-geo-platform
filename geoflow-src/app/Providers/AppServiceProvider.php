...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function register(): void
    {
        ...
        $this->app->bind(AiVisibilityCollector::class, AiVisibilityCollectionService::class);
    ...
    public function boot(): void
    {
        ...
        // API password changes do not use the web `admin` guard.  Resolve the
        // identity from AuthenticateApiToken's ApiAuthContext so a failed or
        // compromised bearer token cannot share the guard's anonymous bucket.
        RateLimiter::for('api-admin-sensitive', function (Request $request): array {
            $auth = $request->attributes->get('api_auth');
            $adminId = $auth instanceof ApiAuthContext ? $auth->auditAdminId : 0;
            $tokenId = $auth instanceof ApiAuthContext ? (int) ($auth->token['id'] ?? 0) : 0;

            return [
                Limit::perMinute(5)->by('api-admin-sensitive:admin:'.$adminId),
                Limit::perMinute(5)->by('api-admin-sensitive:token:'.$tokenId),
                Limit::perMinute(10)->by('api-admin-sensitive:ip:'.$request->ip()),
            ];
        });
        ...
        // 2026-09-12 退役：这里原先有一整块 `View::composer('admin.layouts.app', …)`，
        // 往旧 Blade 后台布局注入欢迎弹窗、更新提示与 `adminUiV3`（导航/页签身份/设置导航）。
        // 那个布局随 `resources/views/admin` 一起删除了，composer 永远不会被触发——
        // 它连带把 `AdminUiRegistry` 也拖成了死代码。React 后台自己从 `/api/v1/catalog`
        // 与 `/api/v1/system-updates` 取这些数据。
    }
