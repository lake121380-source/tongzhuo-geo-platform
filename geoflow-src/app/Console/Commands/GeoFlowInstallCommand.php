...
class GeoFlowInstallCommand extends Command
{
    ...
    protected $description = 'Run 桐灼GEO first-install seeders only for an empty database';
    ...
    public function handle(): int
    {
        ...
        if ($existingState instanceof SystemState && ! $force) {
            $this->components->info('桐灼GEO has already been initialized; first-install seeders were skipped.');
        ...
        if ($tablesWithData !== [] && ! $force) {
            ...
            $this->components->warn('Existing application data was detected. 桐灼GEO recorded the installation marker and skipped first-install seeders.');

            return self::SUCCESS;
        }

        $this->components->info($force
            ? 'Running 桐灼GEO first-install seeders with --force.'
            : 'Running 桐灼GEO first-install seeders for an empty database.');
        ...
        try {
            ...
            $this->error('桐灼GEO first-install seeders failed: '.$e->getMessage());

            return self::FAILURE;
        }

        $this->components->info('桐灼GEO installation marker has been written.');
