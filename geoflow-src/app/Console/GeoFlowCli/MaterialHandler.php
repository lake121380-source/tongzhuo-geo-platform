...
final class MaterialHandler
{
    ...
    public function handle(): int
    {
        ...
        if ($action === 'search') {
            $knowledgeBaseId = $this->runtime->positiveId($arguments[2] ?? null, '知识库 ID');
            $query = trim((string) ($this->runtime->context->options['query'] ?? ''));
            if ($query === '') {
                throw new CliException('知识库检索必须提供 --query');
            }

            return $this->runtime->send('material.knowledge-search', ['knowledgeBase' => $knowledgeBaseId], query: [
                'query' => $query,
                'limit' => $this->runtime->integerOption('limit', 8),
                'max_chars' => $this->runtime->integerOption('max-chars', 6000),
                'expected_generation' => $this->runtime->context->options['expected-generation'] ?? null,
                'allow_remote_embedding' => isset($this->runtime->context->options['allow-remote-embedding']) ? 'true' : null,
            ]);
        }
