...
class SiteThemeReplication extends Model
{
    ...
    /**
     * 2026-09-12：原先只看「状态 READY + 合规通过」，不看清单能不能打包。
     * 而 `ThemeReplicationPublishService::publish()` 通过这道判断后**紧接着就调 `createPackage()`**
     * —— 于是一份清单被清空（或 `current_version = 0`）却仍是 READY 的复刻，界面会显示「可发布」，
     * 点下去必然失败。发布的前提就是打得成包，这里对齐到 `canPackage()`。
     */
