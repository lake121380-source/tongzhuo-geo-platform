...
class DistributionChannel extends Model
{
    ...
    public function resolvedSiteSettings(): array
    {
        ...
        $rawSiteName = $stored['site_name'] ?? $this->name ?? '桐灼GEO Target Site';
        $siteName = trim((string) $rawSiteName);

        return [
            'site_name' => $siteName !== '' ? $siteName : '桐灼GEO Target Site',
            'site_subtitle' => trim((string) ($stored['site_subtitle'] ?? '')),
            'site_description' => trim((string) ($stored['site_description'] ?? '由 桐灼GEO 自动分发和管理的目标站点。')),
            'site_keywords' => trim((string) ($stored['site_keywords'] ?? '')),
            'copyright_info' => trim((string) ($stored['copyright_info'] ?? '© '.date('Y').' '.($siteName !== '' ? $siteName : '桐灼GEO Target Site'))),
