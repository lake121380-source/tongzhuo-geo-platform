...
class AiModelController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($actor->isSuperAdmin()) {
            $viewData += [
                ...
                'defaultEmbeddingModelId' => $this->systemSettingsService->defaultEmbeddingModelId(),
                'chunkingConfig' => $this->systemSettingsService->chunkingConfig(),
    ...
    public function store(StoreAiModelRequest $request): RedirectResponse
    {
        ...
        // 质检结论的失效副作用由 AdminAiModelMutationService 统一负责，
        // 这样旧后台与 api/v1 两条写路径不会一条失效、一条不失效。
        $this->mutationService->create(
            $actor,
            $createData,
    ...
    public function update(UpdateAiModelRequest $request, int $modelId): RedirectResponse
    {
        ...
        // 失效由 AdminAiModelMutationService 在成功路径上统一触发（见其 invalidateQualityAfterCommit）。
    ...
    private function buildTestPayload(
                ...
                return [
                    'requests' => [
                        [
                            ...
                            'content' => [
                                'parts' => [
                                    ['text' => $this->formatGeminiRetrievalQuery('桐灼GEO embedding connection test')],
                                ],
                            ],
        ...
        if ($modelType === 'embedding') {
            return [
                'model' => $modelName,
                'input' => '桐灼GEO embedding connection test',
            ];
        }
    ...
    private function activeActor(Request $request): Admin
    {
        ...
        if (! $actor instanceof Admin) {
            $actor = $request->attributes->get('api_execution_admin');
        }
