  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
        ...
        {!canReadProfile ? <PermissionNotice lang={lang} mode="read" requiredScope="account:read" /> : <div className="grid max-w-3xl grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
        ...
        {!hasAuditRead ? <PermissionNotice lang={lang} mode="read" requiredScope="audit:read" /> : <div className="space-y-4">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="text-slate-500"><tr><th className="pb-2">{isZh ? '时间' : 'Time'}</th><th className="pb-2">{isZh ? '管理员' : 'Admin'}</th><th className="pb-2">{isZh ? '动作' : 'Action'}</th><th className="pb-2">{isZh ? '页面/目标' : 'Page / target'}</th><th className="pb-2">IP</th><th className="pb-2">{isZh ? '详情' : 'Details'}</th></tr></thead><tbody className="divide-y divide-slate-800">{activity.map((log) => <tr key={log.id}><td className="py-3 whitespace-nowrap text-slate-400">{formatDate(log.created_at, lang)}</td><td className="py-3 text-slate-200">{log.admin_username}<div className="text-[10px] text-slate-500">{log.admin_role}</div></td><td className="py-3 font-mono text-emerald-300">{log.request_method} {log.action}</td><td className="py-3 text-slate-400">{log.page || '—'}{log.target_type && <div className="text-[10px] text-slate-500">{log.target_type}#{log.target_id || '—'}</div>}</td><td className="py-3 text-slate-500">{log.ip_address || '—'}</td><td className="max-w-[260px] py-3 text-slate-400"><code className="line-clamp-2 break-all">{typeof log.details === 'string' ? log.details : JSON.stringify(log.details || {})}</code></td></tr>)}{activity.length === 0 && <tr><td colSpan={6} className="py-6 text-center text-slate-500">{isZh ? '暂无审计记录' : 'No audit records'}</td></tr>}</tbody></table></div>
