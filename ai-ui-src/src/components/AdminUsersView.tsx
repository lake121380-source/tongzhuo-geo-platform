...
import { adminRoleLabel, adminStatusLabel } from '../api/labels';
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
      ...
      {formOpen && <form onSubmit={(event) => void save(event)} className="grid grid-cols-1 gap-3 rounded-xl border border-indigo-500/30 bg-indigo-950/20 p-4 md:grid-cols-2">
        ...
        {editing && <label className="text-xs text-slate-400">{isZh ? '状态' : 'Status'}<select value={form.status} onChange={(event) => setField('status', event.target.value as UserForm['status'])} disabled={editing.is_super_admin} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white disabled:opacity-50"><option value="active">{isZh ? '启用' : 'Active'}</option><option value="inactive">{isZh ? '停用' : 'Inactive'}</option></select></label>}
