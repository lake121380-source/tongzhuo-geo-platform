...
import { factValueTypeLabel } from '../api/labels';
...
function asWorkbench(value: unknown): Workbench {
  ...
  return {
    ...
    revisionTotal: Number(row.revision_total ?? 0) || undefined,
...
export const KnowledgeFactWorkbench: React.FC<KnowledgeFactWorkbenchProps> = ({
  ...
  /**
   * 「恢复版本」原来是个 `<select>`，**选中即执行**——一次误点就把整个事实库回滚，
   * 没有任何确认步骤。现在先确认（标题里带上版本号，用户能核对选的是哪个）。
   */
  const confirmRestore = async (revisionId: number) => {
    const version = (workbench.revisions || []).find((revision) => revision.id === revisionId)?.version;
    if (!(await confirmDialog({
      title: lang === 'zh' ? `把事实库恢复到 v${version ?? revisionId}？` : `Restore the fact library to v${version ?? revisionId}?`,
      description: lang === 'zh'
        ? '会用这个版本覆盖当前事实与标准答案，当前内容作为新版本留档、可以再恢复回来。'
        : 'This replaces the current facts and canonical answers with that revision; the current content is kept as a new revision and can be restored again.',
