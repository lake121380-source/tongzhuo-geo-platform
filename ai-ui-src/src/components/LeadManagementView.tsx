    ...
    <div className="space-y-8">
      <div className={`flex flex-col gap-4 lg:flex-row lg:items-end ${embedded ? 'lg:justify-end' : 'lg:justify-between'}`}>
        <PageHeader
          ...
          title={lang === 'zh' ? '线索' : 'Leads'}
          description={lang === 'zh' ? '官网表单收到的人都是谁、从哪来、跟进到哪一步了。' : 'Who filled in your forms, where they came from, and how follow-up is going.'}
        />
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => mode === 'forms' ? void loadForms() : void loadLeads()} disabled={loading} className="inline-flex h-9 items-center gap-2 rounded-xl border border-slate-700 bg-slate-800/60 px-3.5 text-[13px] font-semibold text-slate-200 hover:bg-slate-800 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />{lang === 'zh' ? '刷新' : 'Refresh'}</button>
          {mode === 'forms' && <button type="button" onClick={() => setEditingForm({ id: 0, name: '', slug: '', status: 'active', description: '', submitButtonLabel: lang === 'zh' ? '提交' : 'Submit', successMessage: '', fields: defaultFields.map((field) => ({ ...field })), submissionsCount: 0, publicPath: '', updatedAt: '' })} disabled={!canWrite} className="inline-flex h-9 items-center gap-2 rounded-xl bg-indigo-600 px-3.5 text-[13px] font-bold text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-4 w-4" />{lang === 'zh' ? '新建表单' : 'New form'}</button>}
          {mode === 'leads' && <button type="button" onClick={() => void exportLeads()} disabled={saving} className="inline-flex h-9 items-center gap-2 rounded-xl bg-indigo-600 px-3.5 text-[13px] font-bold text-white transition hover:bg-indigo-500 disabled:opacity-50"><Download className="h-4 w-4" />{lang === 'zh' ? '导出 CSV' : 'Export CSV'}</button>}
        </div>
      </div>

      <div className="inline-flex rounded-xl bg-slate-800/50 p-1">
        <button type="button" onClick={() => setMode('forms')} className={`rounded-lg px-4 py-2 text-[13px] font-semibold transition ${mode === 'forms' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>{lang === 'zh' ? '表单管理' : 'Forms'}</button>
        <button type="button" onClick={() => setMode('leads')} className={`rounded-lg px-4 py-2 text-[13px] font-semibold transition ${mode === 'leads' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>{lang === 'zh' ? '线索收件箱' : 'Lead inbox'}</button>
      </div>
      ...
      {mode === 'forms' ? (
        <>
          ...
          <div className="flex flex-col gap-3 rounded-2xl bg-slate-900/80 p-5 sm:flex-row">
            <input value={formSearch} onChange={(event) => setFormSearch(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && void loadForms(1)} placeholder={lang === 'zh' ? '搜索名称、标识或说明' : 'Search forms'} className="min-w-0 flex-1 h-10 rounded-xl border border-slate-700 bg-slate-900 px-3 text-[13px] text-white outline-none transition focus:border-indigo-500" />
            <select value={formStatus} onChange={(event) => setFormStatus(event.target.value)} className="h-10 rounded-xl border border-slate-700 bg-slate-900 px-3 text-[13px] text-white outline-none transition focus:border-indigo-500"><option value="">{lang === 'zh' ? '全部状态' : 'All statuses'}</option><option value="active">{lang === 'zh' ? '启用' : 'Active'}</option><option value="inactive">{lang === 'zh' ? '停用' : 'Inactive'}</option></select>
            <button type="button" onClick={() => void loadForms(1)} className="h-9 shrink-0 rounded-xl border border-slate-700 bg-slate-800/60 px-3.5 text-[13px] font-semibold text-slate-200 hover:bg-slate-800">{lang === 'zh' ? '查询' : 'Search'}</button>
          </div>
          <div className="overflow-hidden rounded-2xl bg-slate-900/80">
            <div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-800 text-[13px]"><thead className="border-b border-slate-800 bg-slate-800/40 text-[12.5px] font-semibold text-slate-400"><tr className="text-left"><th className="px-3 py-3.5">{lang === 'zh' ? '表单' : 'Form'}</th><th className="px-3 py-3.5">{lang === 'zh' ? '公开地址' : 'Public URL'}</th><th className="px-3 py-3.5">{lang === 'zh' ? '状态' : 'Status'}</th><th className="px-3 py-3.5">{lang === 'zh' ? '提交数' : 'Submissions'}</th><th className="px-3 py-3.5 text-right">{lang === 'zh' ? '操作' : 'Actions'}</th></tr></thead><tbody className="divide-y divide-slate-800">
              {!loading && forms.length === 0 && <tr><td colSpan={5}><EmptyState compact icon={Inbox} title={lang === 'zh' ? '还没有表单' : 'No forms yet'} description={canWrite ? (lang === 'zh' ? '点右上角「新建表单」，把它嵌到官网就能开始收线索。' : 'Use “New form” in the top-right to start collecting leads.') : (lang === 'zh' ? '需要 leads:write 权限才能新建表单。' : 'You need leads:write to create a form.')} /></td></tr>}
              {forms.map((form) => <tr key={form.id} className="text-slate-200 hover:bg-slate-800/40"><td className="px-3 py-4"><p className="font-semibold text-white">{form.name}</p><p className="mt-1 max-w-md truncate text-[12.5px] text-slate-400">{form.description || '-'}</p></td><td className="px-3 py-4"><a href={form.publicPath} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-mono text-[12.5px] text-indigo-300 hover:text-indigo-200">{form.publicPath}<ExternalLink className="h-3 w-3" /></a></td><td className="px-3 py-4"><span className={`rounded-full px-2 py-1 text-[12px] ${form.status === 'active' ? 'bg-emerald-950 text-emerald-300' : 'bg-slate-800 text-slate-400'}`}>{form.status === 'active' ? (lang === 'zh' ? '启用' : 'Active') : (lang === 'zh' ? '停用' : 'Inactive')}</span></td><td className="px-3 py-4 text-slate-300">{form.submissionsCount}</td><td className="px-3 py-4"><div className="flex justify-end gap-2"><button type="button" onClick={() => setEditingForm(form)} disabled={!canWrite} title={lang === 'zh' ? '编辑' : 'Edit'} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-slate-800 hover:text-white disabled:opacity-30"><Pencil className="h-4 w-4" /></button><button type="button" onClick={() => void changeFormStatus(form)} disabled={!canWrite || saving} className="h-8 rounded-lg border border-slate-700 bg-slate-800/60 px-3 text-[12.5px] font-semibold text-slate-200 transition hover:bg-slate-800 disabled:opacity-30">{form.status === 'active' ? (lang === 'zh' ? '停用' : 'Disable') : (lang === 'zh' ? '启用' : 'Enable')}</button><button type="button" onClick={() => void deleteForm(form)} disabled={!canWrite || form.submissionsCount > 0 || saving} title={form.submissionsCount > 0 ? (lang === 'zh' ? '已有线索，只能停用' : 'Disable forms with submissions') : (lang === 'zh' ? '删除' : 'Delete')} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 transition hover:bg-rose-500/10 hover:text-rose-500 disabled:opacity-30"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}
            </tbody></table></div>
        ...
        <>
          ...
          <div className="grid gap-3 rounded-2xl bg-slate-900/80 p-5 md:grid-cols-6">
            <input value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} placeholder={lang === 'zh' ? '搜索提交内容/来源/备注' : 'Search leads'} className="h-10 rounded-xl border border-slate-700 bg-slate-900 px-3 text-[13px] text-white outline-none transition focus:border-indigo-500 md:col-span-2" />
            <select value={filters.status} onChange={(event) => setFilters({ ...filters, status: event.target.value })} className="h-10 rounded-xl border border-slate-700 bg-slate-900 px-3 text-[13px] text-white outline-none transition focus:border-indigo-500"><option value="">{lang === 'zh' ? '全部状态' : 'All statuses'}</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label[lang]}</option>)}</select>
            <select value={filters.formId} onChange={(event) => setFilters({ ...filters, formId: event.target.value })} className="h-10 rounded-xl border border-slate-700 bg-slate-900 px-3 text-[13px] text-white outline-none transition focus:border-indigo-500"><option value="">{lang === 'zh' ? '全部表单' : 'All forms'}</option>{forms.map((form) => <option key={form.id} value={form.id}>{form.name}</option>)}</select>
            <input type="date" value={filters.dateFrom} onChange={(event) => setFilters({ ...filters, dateFrom: event.target.value })} className="h-10 rounded-xl border border-slate-700 bg-slate-900 px-3 text-[13px] text-white outline-none transition focus:border-indigo-500" />
            <input type="date" value={filters.dateTo} onChange={(event) => setFilters({ ...filters, dateTo: event.target.value })} className="h-10 rounded-xl border border-slate-700 bg-slate-900 px-3 text-[13px] text-white outline-none transition focus:border-indigo-500" />
            <div className="flex gap-2 md:col-span-6"><button type="button" onClick={() => { setAppliedFilters(filters); void loadLeads(1, filters); }} className="h-9 rounded-xl border border-slate-700 bg-slate-800/60 px-3.5 text-[13px] font-semibold text-slate-200 transition hover:bg-slate-800">{lang === 'zh' ? '应用筛选' : 'Apply'}</button><button type="button" onClick={() => { const reset = { search: '', status: '', formId: '', dateFrom: '', dateTo: '' }; setFilters(reset); setAppliedFilters(reset); void loadLeads(1, reset); }} className="h-9 rounded-xl px-3.5 text-[13px] font-semibold text-slate-300 transition hover:bg-slate-800 hover:text-white">{lang === 'zh' ? '重置' : 'Reset'}</button></div>
          </div>
          <div className="overflow-hidden rounded-2xl bg-slate-900/80"><div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-800 text-[13px]"><thead className="border-b border-slate-800 bg-slate-800/40 text-[12.5px] font-semibold text-slate-400"><tr className="text-left"><th className="px-3 py-3.5">{lang === 'zh' ? '提交内容' : 'Submission'}</th><th className="px-3 py-3.5">{lang === 'zh' ? '表单' : 'Form'}</th><th className="px-3 py-3.5">{lang === 'zh' ? '状态' : 'Status'}</th><th className="px-3 py-3.5">{lang === 'zh' ? '来源' : 'Source'}</th><th className="px-3 py-3.5">{lang === 'zh' ? '时间' : 'Created'}</th></tr></thead><tbody className="divide-y divide-slate-800">
            {!loading && leads.length === 0 && <tr><td colSpan={5}><EmptyState compact icon={Inbox} title={lang === 'zh' ? '没有符合条件的线索' : 'No matching leads'} description={lang === 'zh' ? '试试放宽筛选条件或换个时间范围；也可能是表单还没收到提交。' : 'Try widening the filters; the form may not have received submissions yet.'} /></td></tr>}
            {leads.map((lead) => <tr key={lead.id} onClick={() => void openLead(lead)} className="cursor-pointer text-slate-300 hover:bg-slate-800/40"><td className="px-3 py-4"><div className="space-y-1">{Object.entries(lead.payload).slice(0, 3).map(([key, value]) => <p key={key} className="max-w-sm truncate"><span className="text-slate-500">{key}：</span>{typeof value === 'boolean' ? (value ? '✓' : '✕') : value}</p>)}</div></td><td className="px-3 py-4">{lead.form?.name || (lang === 'zh' ? '已删除表单' : 'Deleted form')}</td><td className="px-3 py-4"><span className="rounded-full bg-indigo-950 px-2 py-1 text-[12px] text-indigo-300">{statusLabels[lead.status]?.[lang] || lead.status}</span></td><td className="max-w-xs truncate px-3 py-4 text-[12.5px] text-slate-400">{lead.hostedSite?.hostname || lead.sourceUrl || '-'}</td><td className="whitespace-nowrap px-3 py-4 text-[12.5px] text-slate-400">{displayDate(lead.createdAt)}</td></tr>)}
...
const Stat: React.FC<{ label: string; value: number; tone?: 'indigo' | 'emerald' | 'amber' }> = ({ label, value, tone = 'indigo' }) => <div className="rounded-2xl bg-slate-900/80 p-5"><p className="text-caption">{label}</p><p className={`mt-2 text-2xl font-bold ${tone === 'emerald' ? 'text-emerald-300' : tone === 'amber' ? 'text-amber-300' : 'text-indigo-300'}`}>{value}</p></div>;

const Pagination: React.FC<{ meta: PageMeta; lang: Lang; onPage: (page: number) => void }> = ({ meta, lang, onPage }) => <div className="flex items-center justify-between border-t border-slate-800 px-4 py-3 text-[12.5px] text-slate-400"><span>{lang === 'zh' ? `共 ${meta.total} 条` : `${meta.total} total`}</span><div className="flex items-center gap-2"><button type="button" onClick={() => onPage(meta.page - 1)} disabled={meta.page <= 1} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-700 text-slate-400 transition hover:bg-slate-800 hover:text-white disabled:opacity-30"><ChevronLeft className="h-4 w-4" /></button><span>{meta.page} / {meta.totalPages}</span><button type="button" onClick={() => onPage(meta.page + 1)} disabled={meta.page >= meta.totalPages} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-700 text-slate-400 transition hover:bg-slate-800 hover:text-white disabled:opacity-30"><ChevronRight className="h-4 w-4" /></button></div></div>;
