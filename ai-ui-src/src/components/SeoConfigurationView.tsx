...
export const SeoConfigurationView: React.FC<SeoConfigurationViewProps> = ({ apiClient, lang, initialTab = 'llms', canRead = true, canWrite = true }) => {
  ...
  if (!canRead) return (
    <div className="space-y-5">
      <div><h1 className="text-xl font-black text-white flex items-center gap-2"><Globe className="w-5 h-5 text-indigo-400" />SEO 与 AI 抓取输出</h1><p className="text-xs text-slate-400 mt-1">robots.txt、sitemap.xml 和 llms.txt 使用同一后端发布配置。</p></div>
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          {canWrite && <button onClick={() => void save()} disabled={busy !== ''} className="text-xs rounded bg-slate-800 px-3 py-2 text-slate-200 flex items-center gap-1"><Save className="w-3 h-3" />保存草稿</button>}
          {tab === 'sitemap' && canWrite && <button onClick={() => void rebuildSitemap()} disabled={busy !== ''} className="text-xs rounded bg-slate-700 px-3 py-2 text-slate-200 flex items-center gap-1"><RefreshCw className={'w-3 h-3 ' + (busy === 'rebuild' ? 'animate-spin' : '')} />重新生成预览</button>}
          {canWrite && <button onClick={() => void publish()} disabled={busy !== ''} className="text-xs rounded bg-indigo-600 px-3 py-2 text-white flex items-center gap-1"><Upload className="w-3 h-3" />发布输出</button>}
        </div>
      </div>
      {!canWrite && <PermissionNotice lang={lang} requiredScope="seo:write" />}
      {message && <div className="rounded border border-indigo-500/30 bg-indigo-500/10 px-3 py-2 text-xs text-indigo-200">{message}{publishedAt && <span className="ml-2 text-slate-400">最后发布：{publishedAt}</span>}</div>}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <div className="lg:col-span-2 rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-3">
          <label className="block text-xs text-slate-400">站点标题<input disabled={!canWrite} value={config.site_title} onChange={(e) => setConfig({ ...config, site_title: e.target.value })} className="mt-1 w-full rounded bg-slate-950 border border-slate-700 px-3 py-2 text-xs text-white disabled:opacity-60" /></label>
          <label className="block text-xs text-slate-400">站点摘要<textarea disabled={!canWrite} value={config.summary} onChange={(e) => setConfig({ ...config, summary: e.target.value })} rows={3} className="mt-1 w-full rounded bg-slate-950 border border-slate-700 px-3 py-2 text-xs text-white disabled:opacity-60" /></label>
          <label className="flex items-center justify-between text-xs text-slate-300">默认允许抓取<input disabled={!canWrite} type="checkbox" checked={config.allow_all_by_default} onChange={(e) => setConfig({ ...config, allow_all_by_default: e.target.checked })} /></label>
          <label className="flex items-center justify-between text-xs text-slate-300">启用 sitemap<input disabled={!canWrite} type="checkbox" checked={config.include_sitemap} onChange={(e) => setConfig({ ...config, include_sitemap: e.target.checked })} /></label>
          <label className="flex items-center justify-between text-xs text-slate-300">启用 llms.txt<input disabled={!canWrite} type="checkbox" checked={config.include_llms_txt} onChange={(e) => setConfig({ ...config, include_llms_txt: e.target.checked })} /></label>
          <div><div className="text-xs text-slate-400 mb-1">AI 引用规则</div><div className="flex gap-1"><input disabled={!canWrite} value={directiveDraft} onChange={(e) => setDirectiveDraft(e.target.value)} className="flex-1 rounded bg-slate-950 border border-slate-700 px-2 py-1.5 text-xs text-white disabled:opacity-60" /><button disabled={!canWrite} onClick={() => { if (directiveDraft.trim()) setConfig({ ...config, core_directives: [...config.core_directives, directiveDraft.trim()] }); setDirectiveDraft(''); }} className="text-xs rounded bg-slate-800 px-2 disabled:opacity-40">添加</button></div><div className="mt-2 space-y-1">{config.core_directives.map((item, index) => <div key={index} className="flex justify-between text-xs text-slate-300 bg-slate-950 rounded px-2 py-1"><span>{item}</span>{canWrite && <button onClick={() => setConfig({ ...config, core_directives: config.core_directives.filter((_, i) => i !== index) })} className="text-rose-300">×</button>}</div>)}</div></div>
        </div>
        <div className="lg:col-span-3 rounded-xl border border-slate-800 bg-slate-950 overflow-hidden">
