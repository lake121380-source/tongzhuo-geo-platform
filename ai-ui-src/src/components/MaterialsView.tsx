...
export const MaterialsView: React.FC<MaterialsViewProps> = ({
  ...
  const [activeType, setActiveType] = useState<MaterialType>('categories');
  ...
  return (
    <div className="space-y-5 pb-8">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-300 hover:border-slate-600 disabled:opacity-50"
          >
      ...
      {notice && <div className="rounded-lg border border-indigo-500/30 bg-indigo-500/10 px-3 py-2 text-xs text-indigo-200">{notice}</div>}
        ...
        {MATERIAL_TYPES.map((type) => {
          ...
          return (
            <button
              ...
              className={`rounded-xl border p-3 text-left transition ${activeType === type ? 'border-indigo-500/70 bg-indigo-500/10' : 'border-slate-800 bg-slate-900/70 hover:border-slate-700'}`}
            >
              <div className="flex items-center justify-between gap-2">
                <Icon className={`h-4 w-4 ${activeType === type ? 'text-indigo-300' : 'text-slate-400'}`} />
                <span className="text-lg font-bold tabular-nums text-white">{summaryCount(type)}</span>
              </div>
              <div className="mt-2 truncate text-[11px] text-slate-400">{labels[type][lang]}</div>
            </button>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-12">
        <section className="min-h-[420px] rounded-2xl border border-slate-800 bg-slate-900/80 p-4 xl:col-span-5">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-2">
              <ActiveIcon className="h-4 w-4 shrink-0 text-indigo-400" />
              <h2 className="truncate text-sm font-bold text-white">{label}</h2>
              <span className="text-[11px] text-slate-500">{counts[activeType]}</span>
            </div>
          ...
          {loading && activeRecords.length === 0 ? (
            <div className="flex min-h-64 items-center justify-center text-xs text-slate-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" />{lang === 'zh' ? '加载中…' : 'Loading…'}</div>
          ) : visibleRecords.length === 0 ? (
            <div className="flex min-h-64 flex-col items-center justify-center gap-2 text-center text-xs text-slate-500">
              <FolderKanban className="h-8 w-8 text-slate-700" />
              <span>{search ? (lang === 'zh' ? '没有匹配结果' : 'No matching results') : (lang === 'zh' ? '暂无数据，点击右上角新建' : 'No data yet; create one to begin')}</span>
            </div>
          ) : (
            <div className="max-h-[620px] space-y-2 overflow-y-auto pr-1">
              {visibleRecords.map((record) => {
                ...
                return (
                  <div key={id} className={`group flex items-start gap-2 rounded-xl border p-3 transition ${selected ? 'border-indigo-500/60 bg-indigo-500/10' : 'border-slate-800 bg-slate-950/50 hover:border-slate-700'}`}>
                    <button type="button" onClick={() => setSelectedId(id)} className="min-w-0 flex-1 text-left">
                      <div className="flex items-center gap-2">
                        <span className="truncate text-xs font-semibold text-slate-100">{text(record, 'name') || text(record, 'title') || `#${id}`}</span>
                        {activeType === 'categories' && <span className="shrink-0 text-[10px] text-slate-500">{number(record, 'article_count')} 篇</span>}
                        {activeType === 'authors' && <span className="shrink-0 text-[10px] text-slate-500">{number(record, 'article_count')} 篇</span>}
                        {activeType === 'keyword-libraries' && <span className="shrink-0 text-[10px] text-indigo-300">{number(record, 'item_count') || number(record, 'keyword_count')} 词</span>}
                        {activeType === 'title-libraries' && <span className="shrink-0 text-[10px] text-indigo-300">{number(record, 'item_count') || number(record, 'title_count')} 条</span>}
                        {activeType === 'knowledge-bases' && <span className="shrink-0 text-[10px] text-emerald-300">{number(record, 'chunk_count')} 切片</span>}
                      </div>
                      <p className="mt-1 line-clamp-2 text-[11px] leading-relaxed text-slate-500">{text(record, 'description') || text(record, 'email') || (lang === 'zh' ? '暂无描述' : 'No description')}</p>
                      <div className="mt-2 text-[10px] text-slate-600">{formatDate(record.updated_at || record.created_at)}</div>
                    </button>
                    {canWrite && (
                      <div className="flex shrink-0 items-center gap-1 opacity-70 transition group-hover:opacity-100">
                        <button type="button" title={lang === 'zh' ? '编辑' : 'Edit'} onClick={() => beginEdit(record)} className="rounded p-1.5 text-slate-400 hover:bg-slate-800 hover:text-white">✎</button>
                        <button type="button" title={lang === 'zh' ? '删除' : 'Delete'} onClick={() => void deleteMaterial(record)} disabled={busy === `delete-${id}`} className="rounded p-1.5 text-rose-400 hover:bg-rose-950/50 disabled:opacity-40"><Trash2 className="h-3.5 w-3.5" /></button>
                      </div>
                    )}
        ...
        <section className="space-y-5 xl:col-span-7">
          {(formOpen && canWrite) && (
            <form onSubmit={saveMaterial} className="rounded-2xl border border-indigo-500/30 bg-slate-900/80 p-5">
              <div className="mb-4 flex items-center justify-between border-b border-slate-800 pb-3">
                <h3 className="text-sm font-bold text-white">{editing ? (lang === 'zh' ? `编辑${label}` : `Edit ${label}`) : (lang === 'zh' ? `新建${label}` : `New ${label}`)}</h3>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                {activeType === 'knowledge-bases' && <label className="text-xs text-slate-400">{lang === 'zh' ? '文件类型' : 'File type'}<select value={form.file_type} onChange={(event) => setField('file_type', event.target.value)} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white"><option value="markdown">Markdown</option><option value="text">Text</option><option value="word">Word</option></select></label>}
                <label className="text-xs text-slate-400 sm:col-span-2">{lang === 'zh' ? '描述' : 'Description'}<textarea value={form.description} onChange={(event) => setField('description', event.target.value)} rows={3} className="mt-1 w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white outline-none focus:border-indigo-500" /></label>
                {activeType === 'authors' && <label className="text-xs text-slate-400 sm:col-span-2">{lang === 'zh' ? '简介' : 'Bio'}<textarea value={form.bio} onChange={(event) => setField('bio', event.target.value)} rows={3} className="mt-1 w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white outline-none focus:border-indigo-500" /></label>}
                {activeType === 'authors' && <label className="text-xs text-slate-400 sm:col-span-2">{lang === 'zh' ? '社交链接（JSON 或文本）' : 'Social links (JSON or text)'}<textarea value={form.social_links} onChange={(event) => setField('social_links', event.target.value)} rows={2} className="mt-1 w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white outline-none focus:border-indigo-500" /></label>}
                {activeType === 'knowledge-bases' && (!editing || form.content !== '') && <label className="text-xs text-slate-400 sm:col-span-2">{lang === 'zh' ? '正文' : 'Content'}<textarea required={!editing} value={form.content} onChange={(event) => setField('content', event.target.value)} rows={8} className="mt-1 w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 font-mono text-xs text-white outline-none focus:border-indigo-500" /></label>}
              </div>
              <div className="mt-4 flex justify-end gap-2 border-t border-slate-800 pt-3">
                <button type="button" onClick={() => { setEditing(null); setFormOpen(false); setForm({ ...emptyForm }); }} className="rounded-lg bg-slate-800 px-3 py-2 text-xs text-slate-300 hover:bg-slate-700">{lang === 'zh' ? '取消' : 'Cancel'}</button>
                <button type="submit" disabled={busy === 'material'} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-4 py-2 text-xs font-semibold text-white hover:bg-indigo-500 disabled:opacity-50"><Plus className="h-3.5 w-3.5" />{busy === 'material' ? (lang === 'zh' ? '保存中…' : 'Saving…') : (lang === 'zh' ? '保存' : 'Save')}</button>
              </div>
            </form>
          )}
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
            <div className="mb-4 flex items-start justify-between gap-3 border-b border-slate-800 pb-3">
              <div className="min-w-0">
                <h3 className="truncate text-sm font-bold text-white">{activeRecord ? text(activeRecord, 'name') : label}</h3>
                <p className="mt-1 text-[11px] text-slate-500">{activeRecord ? (text(activeRecord, 'description') || (lang === 'zh' ? '暂无描述' : 'No description')) : (lang === 'zh' ? '选择左侧素材库查看详情' : 'Select a library to inspect its details')}</p>
              </div>
              {activeRecord && <span className="shrink-0 rounded bg-slate-800 px-2 py-1 text-[10px] text-slate-400">ID {materialId(activeRecord)}</span>}
            </div>
            ...
            {activeRecord && activeItemsEnabled && (
              <div className="space-y-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="text-xs font-semibold text-slate-300">{activeType === 'keyword-libraries' ? (lang === 'zh' ? '关键词条目' : 'Keywords') : (lang === 'zh' ? '标题条目' : 'Titles')} <span className="text-slate-500">({items.length})</span></div>
                  <div className="flex items-center gap-2">
                  {canWrite && <button type="button" onClick={() => setImportOpen(true)} className="inline-flex items-center gap-1 rounded-lg border border-indigo-500/40 px-2.5 py-1.5 text-[11px] text-indigo-300 hover:bg-indigo-950/30"><Upload className="h-3 w-3" />{lang === 'zh' ? '批量导入' : 'Bulk import'}</button>}
                  {canWrite && selectedItemIds.size > 0 && <button type="button" onClick={() => void deleteItems()} disabled={busy === 'item-delete'} className="inline-flex items-center gap-1 rounded-lg border border-rose-500/40 px-2.5 py-1.5 text-[11px] text-rose-300 hover:bg-rose-950/30 disabled:opacity-50"><Trash2 className="h-3 w-3" />{lang === 'zh' ? `删除所选 (${selectedItemIds.size})` : `Delete selected (${selectedItemIds.size})`}</button>}
                </div>
                {canWrite && <form onSubmit={createItem} className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div className="flex flex-col gap-2 sm:flex-row"><input value={activeType === 'keyword-libraries' ? itemDraft.keyword : itemDraft.title} onChange={(event) => setItemDraft((previous) => activeType === 'keyword-libraries' ? { ...previous, keyword: event.target.value } : { ...previous, title: event.target.value })} placeholder={activeType === 'keyword-libraries' ? (lang === 'zh' ? '输入关键词' : 'Keyword') : (lang === 'zh' ? '输入标题' : 'Title')} className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white outline-none focus:border-indigo-500" />{activeType === 'title-libraries' && <input value={itemDraft.relatedKeyword} onChange={(event) => setItemDraft((previous) => ({ ...previous, relatedKeyword: event.target.value }))} placeholder={lang === 'zh' ? '关联关键词（可选）' : 'Related keyword (optional)'} className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-white outline-none focus:border-indigo-500" />}<button type="submit" disabled={busy === 'item-create'} className="inline-flex items-center justify-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-500 disabled:opacity-50"><Plus className="h-3.5 w-3.5" />{lang === 'zh' ? '添加' : 'Add'}</button></div></form>}
                  </div>
                ...
                {itemError && <div role="alert" className="rounded-lg border border-rose-500/30 bg-rose-950/30 px-3 py-2 text-xs text-rose-200">{itemError}</div>}
                {itemsLoading ? <div className="flex min-h-32 items-center justify-center text-xs text-slate-500"><Loader2 className="mr-2 h-4 w-4 animate-spin" />{lang === 'zh' ? '读取条目…' : 'Loading items…'}</div> : items.length === 0 ? <div className="rounded-xl border border-dashed border-slate-800 px-4 py-10 text-center text-xs text-slate-500">{lang === 'zh' ? '此库暂无条目' : 'This library has no items yet'}</div> : <div className="max-h-[390px] space-y-2 overflow-y-auto pr-1">{items.map((item) => { const id = materialId(item); const itemText = activeType === 'keyword-libraries' ? text(item, 'keyword') : text(item, 'title'); return <label key={id} className="flex cursor-pointer items-start gap-2 rounded-lg border border-slate-800 bg-slate-950/50 p-3 hover:border-slate-700"><input type="checkbox" checked={selectedItemIds.has(id)} onChange={() => toggleItem(id)} className="mt-0.5 accent-indigo-500" /><span className="min-w-0 flex-1"><span className="block break-words text-xs text-slate-200">{itemText || `#${id}`}</span>{activeType === 'title-libraries' && text(item, 'keyword') && <span className="mt-1 block text-[10px] text-slate-500">{lang === 'zh' ? '关键词：' : 'Keyword: '}{text(item, 'keyword')}</span>}<span className="mt-1 block text-[10px] text-slate-600">{formatDate(item.created_at)}</span></span></label>; })}</div>}
              </div>
            )}

            {!activeRecord && !formOpen && <div className="flex min-h-48 items-center justify-center text-xs text-slate-500">{lang === 'zh' ? '暂无可查看的素材库' : 'No library selected'}</div>}
          </div>
        </section>
      </div>
...
const PageHeading: React.FC<{ lang: 'zh' | 'en' }> = ({ lang }) => (
  <div>
    <h1 className="flex items-center gap-2 text-xl font-black text-white sm:text-2xl"><FolderKanban className="h-6 w-6 text-indigo-400" />{lang === 'zh' ? '素材库管理' : 'Material libraries'}</h1>
    <p className="mt-1 text-xs text-slate-400">{lang === 'zh' ? '分类、作者、关键词、标题与知识资产均来自 桐灼GEO 真实后端。' : 'Categories, authors, keywords, titles and knowledge assets are backed by 桐灼GEO API v1.'}</p>
  </div>
);
...
const Field: React.FC<{ label: string; value: string; onChange: (value: string) => void; required?: boolean; type?: string }> = ({ label, value, onChange, required = false, type = 'text' }) => (
  <label className="text-xs text-slate-400">{label}{required && <span className="ml-1 text-rose-400">*</span>}<input required={required} type={type} value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-white outline-none focus:border-indigo-500" /></label>
);

const Stat: React.FC<{ label: string; value: number }> = ({ label, value }) => (
  <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div className="text-[10px] text-slate-500">{label}</div><div className="mt-1 text-lg font-bold tabular-nums text-white">{value}</div></div>
);
