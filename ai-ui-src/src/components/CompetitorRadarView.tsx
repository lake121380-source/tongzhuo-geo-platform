...
interface CompetitorRadarViewProps { lang: 'zh' | 'en'; onNavigateToDraft?: (topic: string) => void; apiClient?: GeoFlowApiClient; canRead?: boolean; canCollect?: boolean; }
...
export const CompetitorRadarView: React.FC<CompetitorRadarViewProps> = ({ lang, onNavigateToDraft, apiClient, canRead = false, canCollect = false }) => {
  ...
  return <div className="space-y-6" id="competitor-radar-container">
    ...
    <div className="flex flex-col gap-4 rounded-xl border border-slate-800 bg-slate-900/90 p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between"><div><div className="flex items-center gap-2 text-xs text-indigo-300"><Database className="h-4 w-4" />{lang === 'zh' ? '真实 AI 可见性引用投影' : 'Persisted AI visibility citation projection'}</div><h2 className="mt-2 text-xl font-bold text-white">{lang === 'zh' ? '竞品 GEO 引用雷达' : 'Competitor GEO Citation Radar'}</h2><p className="mt-1 text-sm text-slate-400">{lang === 'zh' ? '指标只来自时间窗内已持久化的模型回答与引用记录。' : 'Metrics use persisted model answers and citations within the selected window.'}</p></div><div className="flex flex-wrap items-end gap-2"><label className="text-xs text-slate-400">{lang === 'zh' ? '时间窗' : 'Window'}<select value={days} onChange={(e) => setDays(Number(e.target.value))} className="mt-1 block rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-white"><option value={7}>7 {lang === 'zh' ? '天' : 'days'}</option><option value={30}>30 {lang === 'zh' ? '天' : 'days'}</option><option value={90}>90 {lang === 'zh' ? '天' : 'days'}</option></select></label><button onClick={() => setShowAddModal(true)} disabled={!canCollect || busy} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800 px-3.5 py-2 text-xs font-medium text-slate-200 disabled:opacity-50"><Plus className="h-3.5 w-3.5" />{lang === 'zh' ? '添加竞品' : 'Add competitor'}</button></div></div>
